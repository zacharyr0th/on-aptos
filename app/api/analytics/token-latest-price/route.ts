  ...
  try {
    ...
    try {
      ...
      if (panoraPrices && panoraPrices.length > 0) {
        ...
        if (panoraPrice.usdPrice && panoraPrice.symbol) {
          ...
          apiLogger.debug(`Token price fetched from Panora: $${panoraPrice.usdPrice} (took ${Date.now() - startTime}ms)`);
          return NextResponse.json(
            { data, source: "panora" },
            {
              headers: {
          ...
          apiLogger.debug("Panora returned null/invalid data, falling back to Aptos Analytics");
        }
      }
    } catch (panoraError) {
      apiLogger.error(`Failed to fetch price from Panora: ${panoraError instanceof Error ? panoraError.message : String(panoraError)}`);
    }
    ...
    try {
      ...
      if (data && data.length > 0) {
        apiLogger.debug(`Token price fetched from Aptos Analytics (took ${Date.now() - startTime}ms)`);
        return NextResponse.json(
      ...
      apiLogger.error(`Failed to fetch price from Aptos Analytics: ${aptosError instanceof Error ? aptosError.message : String(aptosError)}`);
    }

    // If both fail, return empty data
    apiLogger.warn(`No price data found for token (took ${Date.now() - startTime}ms)`);
    return NextResponse.json(
      {
        data: [],
    ...
    apiLogger.error(`Token latest price API error: ${error instanceof Error ? error.message : String(error)}`);
    return NextResponse.json(
      {
