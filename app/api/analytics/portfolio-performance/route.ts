...
export async function GET(request: NextRequest) {
  try {
        ...
        try {
          ...
          if (balanceHistoryResponse.ok) {
            ...
            if (
              ...
              apiLogger.info("[Portfolio Performance] Using balance-history API data");
          ...
          apiLogger.warn({ error: altError }, "[Portfolio Performance] Alternative balance fetch failed");
        }
      }
    }
    ...
    if (!balanceData || balanceData.length === 0) {
      apiLogger.warn({ walletAddress }, "[Portfolio Performance] No balance data returned for wallet");
      return NextResponse.json({
        success: true,
        data: [],
        timeframe,
    ...
    if (performanceData.length > 1) {
      ...
      if (uniqueValues.length <= 1) {
        apiLogger.info("[Portfolio Performance] Wallet has flat balance history - returning real data");
      }
    }
...
async function fetchBalanceHistory(walletAddress: string, config: any) {
  ...
  if (!response.ok) {
    ...
    apiLogger.error(`[fetchBalanceHistory] API error: ${response.status} - ${errorText}`);
  ...
  if (result.status !== "success" || !result.data) {
    apiLogger.error(`[fetchBalanceHistory] Unexpected response: ${result instanceof Error ? result.message : JSON.stringify(result)}`);
    throw new Error("Failed to fetch balance data");
  }

  apiLogger.info(`[Portfolio Performance] Balance data sample: ${result.data.length} items`);
...
async function fetchPriceHistory(config: any) {
  ...
  apiLogger.info(`[Portfolio Performance] Price data sample: ${result.data.length} items`);
