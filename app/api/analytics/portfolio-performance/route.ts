...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!balanceData || balanceData.length === 0) {
      logger.warn('[Portfolio Performance] No balance data returned for wallet:', walletAddress);
      return NextResponse.json({
        success: true,
        data: [],
        timeframe,
...
async function fetchBalanceHistory(walletAddress: string, config: any) {
  ...
  if (!response.ok) {
    ...
    logger.error('[fetchBalanceHistory] API error:', { status: response.status, error: errorText });
...
function combineData(balanceData: any[], priceData: any[], config: any): PerformanceData[] {
  // Create maps for easier lookup
  const priceMap = new Map();
  priceData.forEach((item: any) => {
    // Handle different timestamp fields based on lookback period
    const timestamp = item.price_hourly_timestamp || 
  ...
  allBalances.forEach((item: any) => {
    ...
    const timestamp = item.date_day || item.hourly_timestamp || item.date || item.timestamp;
    const value = item.total_balance_usd || item.total_store_balance_usd || item.balance_usd || 0;
    if (timestamp) {
  ...
  if (config.granularity === 'minutes' || config.granularity === 'hourly' || config.days <= 90) {
    // For short timeframes, use price data points to maintain granularity
    priceData.forEach((priceItem: any) => {
      const priceTimestamp = priceItem.price_hourly_timestamp || 
    ...
    allBalances.forEach((balanceItem: any) => {
      const timestamp = balanceItem.date_day || balanceItem.hourly_timestamp || balanceItem.date || balanceItem.timestamp;
      const totalValue = balanceItem.total_balance_usd || balanceItem.total_store_balance_usd || balanceItem.balance_usd || 0;
  ...
  if (config.days < 999999) {  // Not 'all' timeframe
    const cutoffDate = new Date();
    cutoffDate.setTime(cutoffDate.getTime() - (config.days * 24 * 60 * 60 * 1000));
