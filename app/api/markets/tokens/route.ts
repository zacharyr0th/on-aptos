...
async function fetchPanoraPrices(): Promise<Map<string, string>> {
  ...
  try {
    ...
    const prices =
      pricesResponse?.data && Array.isArray(pricesResponse.data) ? pricesResponse.data : [];
...
function processTokensFromPanora(tokens: TokenInfo[]): TokenInfo[] {
  ...
  const aptToken = tokens.find(
    (t) => t.symbol === "APT" || t.tokenAddress?.toLowerCase() === APT_ADDRESS.toLowerCase()
  );
 ...
 */
async function fetchTokenMetadata(addresses: string[]): Promise<Map<string, any>> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Object.entries(data.data || {}).forEach(([key, value]: [string, any]) => {
          ...
          } else if (key.startsWith("fa_metadata_") && Array.isArray(value) && value[0]) {
        ...
        apiLogger.warn(`GraphQL request failed with status: ${response.status}`);
      }
    } catch (error) {
  ...
  apiLogger.info(`Successfully fetched metadata for ${metadataMap.size} tokens`);
  return metadataMap;
}
 ...
 */
function sortAndPaginateTokens(tokens: TokenInfo[], limit: number, offset: number): TokenInfo[] {
...
function createTokenMarketData(
  ...
  const isVerified = token.panoraTags?.some((tag) => tag === "Verified") || false;
...
async function processTokensBatch(
  ...
  return tokens.map((token, index) => {
    ...
    const metadata = metadataMap.get(token.faAddress || token.tokenAddress || "");
    const rawSupply = metadata?.supply;
    const decimals = metadata?.decimals !== undefined ? metadata.decimals : token.decimals;
    ...
    if (rawSupply) {
      try {
        ...
        apiLogger.warn(`Failed to parse supply for ${token.symbol}: ${rawSupply}`, { error });
        // Fallback to simpler parsing
        supply = parseInt(rawSupply) / 10 ** decimals;
      }
    }
...
async function calculateMarketCaps(
  ...
  try {
    ...
    const additionalTokens = allTokens.filter(
      (token: any) => !uiAddresses.has(token.faAddress) && !uiAddresses.has(token.tokenAddress)
    );
    ...
    if (!fetchAll) {
      ...
      return {
        ...
        totalMarketCap: allProcessedTokens.reduce((sum, t) => sum + (t.marketCap || 0), 0),
        aptPrice: allProcessedTokens.find((t) => t.symbol === "APT")?.price
          ? parseFloat(allProcessedTokens.find((t) => t.symbol === "APT")!.price!)
          : 0,
...
function parseTokensParams(request: NextRequest) {
  ...
  return {
    ...
    offset: parseNumericParam(request.nextUrl.searchParams.get("offset"), 0, 0, 10000),
