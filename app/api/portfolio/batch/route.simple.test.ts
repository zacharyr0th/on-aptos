...
describe('GET /api/portfolio/batch - Simple Test', () => {
  it('should return 400 when wallet address is missing', async () => {
    const request = new NextRequest('http://localhost:3000/api/portfolio/batch')
  ...
  it('should return portfolio data when address is provided', async () => {
    ...
    const request = new NextRequest(`http://localhost:3000/api/portfolio/batch?walletAddress=${validAddress}`)
    const response = await GET(request)
    const data = await response.json()
