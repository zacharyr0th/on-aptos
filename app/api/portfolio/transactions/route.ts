...
async function fetchFromGraphQL(address: string, requestedLimit: number, startOffset: number = 0) {
  ...
  apiLogger.info(`Fetching ${requestedLimit} transactions starting at offset ${startOffset} for address ${address}`);
  ...
  const totalCount = result.data?.account_transactions_aggregate?.aggregate?.count || 0;
  
  apiLogger.info(`Fetched ${accountTxns.length} transaction versions out of ${totalCount} total`);
  ...
  for (let i = 0; i < txVersions.length; i += detailsBatchSize) {
    ...
    allBlockMetadata = allBlockMetadata.concat(detailsResult.data?.block_metadata_transactions || []);
    
    // Log progress for batches
    if (i > 0) {
      apiLogger.info(`Fetched details for ${i + batchVersions.length}/${txVersions.length} transactions`);
    }
  }
    ...
    for (const event of txEvents) {
      if (event.type && event.data) {
        try {
          ...
          if (event.type.includes('CoinWithdrawEvent') || 
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    apiLogger.info(`Transaction fetch completed: ${transactions.length} transactions, ${totalCount} total`);
