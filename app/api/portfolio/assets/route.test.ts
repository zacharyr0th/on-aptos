...
describe('GET /api/portfolio/assets', () => {
  ...
  it('should return 400 when address is invalid', async () => {
    const request = new Request('http://localhost:3000/api/portfolio/assets?walletAddress=invalid')
    const response = await GET(request)
    const data = await response.json()
  ...
  it('should return assets when address is valid', async () => {
    ...
    const request = new Request(`http://localhost:3000/api/portfolio/assets?walletAddress=${validAddress}`)
    const response = await GET(request)
    const data = await response.json()
