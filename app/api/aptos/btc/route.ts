...
async function queryGraphQL(query: string, variables: Record<string, unknown>) {
  ...
  try {
    logger.info("[BTC API] Executing GraphQL query");
    ...
    logger.info("[BTC API] GraphQL query completed successfully");
    return result;
  } catch (error: unknown) {
    clearTimeout(timeoutId);
    if (error instanceof Error && error.name === "AbortError") {
      logger.warn("[BTC API] GraphQL query timed out after 30 seconds");
      throw new Error("Request timeout");
    }
    logger.error("[BTC API] GraphQL query failed:", error);
    throw error;
  }
}
        ...
        try {
          ...
          `;
            ...
            `;
              ...
              if (offset > 100000) {
                logger.warn(
                  `[BTC API] Hit safety limit for ${token.symbol}, got ${allBalances.length} records`,
                );
                break;
              }
            }
          ...
          logger.warn(`[BTC API] Failed to fetch ${token.symbol}:`, err);
          return {
            symbol: token.symbol,
            supply: "0",
