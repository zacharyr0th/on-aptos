...
import { createErrorResponse } from '@/lib/utils/response-builder'
...
if (!PANORA_API_KEY) {
  throw new Error('PANORA_API_KEY environment variable is required. Please check your .env file.')
}
if (!APTOS_BUILD_SECRET) {
  throw new Error('APTOS_BUILD_SECRET environment variable is required. Please check your .env file.')
}
const APTOS_GRAPHQL_URL = 'https://api.mainnet.aptoslabs.com/v1/graphql'

// Tether reserve address that should be subtracted from USDT supply
...
async function fetchPanoraPrices(): Promise<PanoraToken[]> {
  try {
    ...
    apiLogger.info(`Fetched ${Array.isArray(verifiedTokens) ? verifiedTokens.length : 0} verified tokens and ${Array.isArray(allTokens) ? allTokens.length : 0} total tokens from Panora`)
    ...
    const tokensWithoutPrices = allAvailableTokens.filter(
      (token: any) => !token.usdPrice || parseFloat(token.usdPrice) <= 0
    ).map((token: any) => ({
      ...token,
      usdPrice: '0'
    ...
    const hasAPT = tokensWithPrices.some(token => 
      token.symbol === 'APT' || 
    ...
    if (!hasAPT) {
      try {
        ...
        const aptResponse = await fetch('https://api.panora.exchange/tokenlist?tokenAddress=0x1::aptos_coin::AptosCoin', {
          headers: {
            'x-api-key': PANORA_API_KEY!,
            'Accept': 'application/json',
            'Accept-Encoding': 'gzip, deflate'
        ...
        if (aptResponse.ok) {
          ...
          if (aptToken && aptToken.usdPrice && parseFloat(aptToken.usdPrice) > 0) {
            tokensWithPrices.push(aptToken)
            apiLogger.info('Added APT token separately since it was not in verified tokens')
          }
        }
      } catch (aptError) {
        apiLogger.warn('Failed to fetch APT token separately', { error: aptError })
      }
    }
...
async function calculateMarketCaps(includeAPT = true, maxTokens = 0): Promise<{
  ...
  const aptToken = allValidTokens.find(t => 
    t.symbol === 'APT' || 
  ...
  let nonAptTokens = allValidTokens.filter(t => 
    t.symbol !== 'APT' && 
    t.tokenAddress !== '0x1::aptos_coin::AptosCoin' &&
  ...
  if (maxTokens > 0) {
    ...
    const verifiedTokens = nonAptTokens.filter(t => t.panoraTags?.includes('Verified'))
    const unverifiedTokens = nonAptTokens.filter(t => !t.panoraTags?.includes('Verified'))
    
    // Take first 500 verified + first 1000 unverified for supply analysis
    nonAptTokens = [...verifiedTokens.slice(0, 500), ...unverifiedTokens.slice(0, 1000)]
  ...
  apiLogger.info(`Processing ${nonAptTokens.length} non-APT tokens and APT separately`)
  ...
  apiLogger.info(`Processing ${faAddresses.length} FA tokens in ${totalBatches} batches`)
  
  for (let i = 0; i < faAddresses.length; i += batchSize) {
    const batchNum = Math.floor(i / batchSize) + 1
    const batch = faAddresses.slice(i, Math.min(i + batchSize, faAddresses.length))
    ...
    try {
      const { metadataMap, usdtReserveBalance } = await fetchTokenMetadata(batch)

      // Calculate market caps for this batch
      for (const token of nonAptTokens.filter(t => batch.includes(t.faAddress as string))) {
        ...
        if (metadata && metadata.supply_v2) {
          ...
          if (token.faAddress === STABLECOINS.USDT && usdtReserveBalance !== '0') {
            const reserveAmount = parseFloat(usdtReserveBalance)
            supply = supply - reserveAmount
            apiLogger.info(`USDT: Total supply ${metadata.supply_v2}, Reserve ${usdtReserveBalance}, Circulating ${supply}`)
          }
          ...
          if ((price === 0 || (price < 1000000 && marketCap >= 0 && marketCap < 1000000000000)) && !isNaN(marketCap) && isFinite(marketCap)) {
            if (price > 0) {
              tokensWithSupply++
      ...
      apiLogger.warn(`Batch ${batchNum}/${totalBatches} failed, continuing...`, { error })
    }
  ...
  apiLogger.info(`Processed ${tokensWithSupply} tokens with supply data out of ${faAddresses.length} FA tokens`)

  // Process legacy tokens (v1 coins) that only have tokenAddress
  const legacyTokens = nonAptTokens.filter(t => 
    t.tokenAddress && t.tokenAddress !== 'null' && 
    (!t.faAddress || t.faAddress === 'null')
  )
  
  apiLogger.info(`Processing ${legacyTokens.length} legacy tokens with tokenAddress only`)
  ...
  const tokensWithoutAddresses = nonAptTokens.filter(t => 
    (!t.faAddress || t.faAddress === 'null') && 
    (!t.tokenAddress || t.tokenAddress === 'null')
  )
  
  apiLogger.info(`Processing ${tokensWithoutAddresses.length} tokens without addresses`)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const maxMarketCap = parseFloat(searchParams.get('max_market_cap') || '1000000000000')
    ...
    const filteredTokens = tokens.filter(t => 
      t.marketCap >= minMarketCap && 
      t.marketCap <= maxMarketCap
    )

    // Calculate market cap distribution
    const distribution = {
      above1B: filteredTokens.filter(t => t.marketCap >= 1000000000).length,
      from100MTo1B: filteredTokens.filter(t => t.marketCap >= 100000000 && t.marketCap < 1000000000).length,
      from10MTo100M: filteredTokens.filter(t => t.marketCap >= 10000000 && t.marketCap < 100000000).length,
      from1MTo10M: filteredTokens.filter(t => t.marketCap >= 1000000 && t.marketCap < 10000000).length,
      from100KTo1M: filteredTokens.filter(t => t.marketCap >= 100000 && t.marketCap < 1000000).length,
      below100K: filteredTokens.filter(t => t.marketCap < 100000).length
    }

    // Calculate statistics
    const avgMarketCap = filteredTokens.length > 0 ? 
      filteredTokens.reduce((sum, t) => sum + t.marketCap, 0) / filteredTokens.length : 0
    
    const medianMarketCap = filteredTokens.length > 0 ?
      filteredTokens[Math.floor(filteredTokens.length / 2)]?.marketCap || 0 : 0
    ...
    const keyTokens = {
      ...
      bitcoin: filteredTokens.filter(t => 
        t.symbol.includes('BTC') || t.symbol.includes('WBTC') || t.symbol === 'aBTC'
      ),
      defi: filteredTokens.filter(t => 
        ['AMI', 'RION', 'THL', 'ECHO', 'LSD', 'CELL'].includes(t.symbol)
      ),
      liquid_staking: filteredTokens.filter(t => 
        t.symbol.includes('APT') && (t.symbol.includes('st') || t.symbol.includes('am'))
      )
    }
    ...
    if (limit > 0) {
      ...
      apiLogger.info(`Returning paginated tokens: offset=${offset}, limit=${limit}, returned=${paginatedTokens.length}`)
    }
      ...
      data: {
        ...
        categories: {
          stablecoins: {
            ...
            total_market_cap: keyTokens.stablecoins.reduce((sum, t) => sum + t.marketCap, 0),
            tokens: keyTokens.stablecoins.slice(0, 5)
          },
          bitcoin: {
            count: keyTokens.bitcoin.length,
            total_market_cap: keyTokens.bitcoin.reduce((sum, t) => sum + t.marketCap, 0),
            tokens: keyTokens.bitcoin.slice(0, 5)
          },
          defi: {
            count: keyTokens.defi.length,
            total_market_cap: keyTokens.defi.reduce((sum, t) => sum + t.marketCap, 0),
            tokens: keyTokens.defi.slice(0, 10)
          },
          liquid_staking: {
            count: keyTokens.liquid_staking.length,
            total_market_cap: keyTokens.liquid_staking.reduce((sum, t) => sum + t.marketCap, 0),
