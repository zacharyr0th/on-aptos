...
const CACHE_TTL = 24 * 60 * 60 * 1000; // 24 hours (daily, maximum cost savings)
...
async function fetchRealTimeRWAData() {
  ...
  try {
    ...
    // apiLogger.info("Fetching RWA data from:", RWA_BASE_URL);
    // apiLogger.info("API Key configured:", !!RWA_API_KEY);
    ...
    // apiLogger.info("Token response status:", tokenResponse.status);
    // apiLogger.info("Asset response status:", assetResponse.status);
    ...
    // apiLogger.info("Token data results:", tokenData.results?.length || 0);
    // apiLogger.info("Asset data results:", assetData.results?.length || 0);
    ...
    // apiLogger.info("Total protocols found:", protocols.length);
    // apiLogger.info("Total RWA value:", totalAptosValue);
...
export async function GET(request: NextRequest) {
  ...
  return withErrorHandling(async () => {
    ...
    try {
      ...
      // apiLogger.error(`RWA API error:: ${error instanceof Error ? error.message : error}`);
