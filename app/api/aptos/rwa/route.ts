...
function setCachedData(key: string, data: unknown, ttl: number = CACHE_TTL): void {
...
async function fetchRealTimeRWAData() {
  ...
  try {
    ...
    apiLogger.debug("Fetching RWA data from:", RWA_BASE_URL);
    apiLogger.debug("API Key configured:", !!RWA_API_KEY);
    ...
    apiLogger.debug("Token response status:", tokenResponse.status);
    apiLogger.debug("Asset response status:", assetResponse.status);
    ...
    if (!tokenResponse.ok || !assetResponse.ok) {
      ...
      apiLogger.error("API Error - Token:", tokenError);
      apiLogger.error("API Error - Asset:", assetError);
      throw new Error(
    ...
    apiLogger.debug("Token data results:", tokenData.results?.length || 0);
    apiLogger.debug("Asset data results:", assetData.results?.length || 0);
    ...
    const totalAptosValue = protocols.reduce((sum, protocol) => sum + protocol.totalValue, 0);

    apiLogger.debug("Total protocols found:", protocols.length);
    apiLogger.debug("Total RWA value:", totalAptosValue);
    ...
    () =>
      withErrorHandling(async () => {
        ...
        try {
          ...
          apiLogger.error("RWA API error:", error);
