...
function isDeFiAsset(asset: any): boolean {
  ...
  // Check for staking tokens (exclude tAPT which is Tortuga)
  if (
  ...
  if (
    DEFI_PATTERNS.DEFI_CONTRACTS.some((contract) =>
      assetType.includes(contract),
    )
  ) {
    return true;
  }

  // Check for staking-related tokens in name (exclude tAPT which is Tortuga)
  if (
    (symbol.toLowerCase().includes("staked") ||
      name.toLowerCase().includes("staked")) &&
    !symbol.includes("tAPT")
  ) {
    return true;
  }
...
function getProtocolInfo(asset: any): {
  ...
  if (
    ...
    assetType.includes(
      "48271d39d0b05bd6efca2278f22277d6fcc375504f9839fd73f74ace240861af",
    )
  ) {
    return {
      protocol: "Thala",
  ...
  // Generic staking (exclude tAPT which is Tortuga)
  if (
...
async function handler(request: NextRequest) {
  ...
  try {
    ...
    if (!portfolioResponse.ok) {
      throw new Error(
        `Failed to fetch portfolio assets: ${portfolioResponse.status}`,
      );
    }
    ...
    const totalValue = defiAssets.reduce(
