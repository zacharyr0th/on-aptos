...
export async function GET() {
  try {
    ...
    apiLogger.info(`Querying fungible assets: ${JSON.stringify(fungibleAssets)}`);
    ...
    if (result.errors) {
      apiLogger.error(`GraphQL errors: ${JSON.stringify(result.errors)}`);
      throw new Error("GraphQL query failed: " + JSON.stringify(result.errors));
    }

    apiLogger.info(`GraphQL response: ${JSON.stringify(result.data?.fungible_asset_metadata, null, 2)}`);
      ...
      for (const item of result.data.fungible_asset_metadata) {
        if (item.supply_v2) {
          ...
          if (symbol === "USDT") {
            ...
            apiLogger.info(`USDT: Total supply ${item.supply_v2}, Reserve ${usdtReserveBalance}, Circulating ${supply.toString()}`);
          }
          ...
          if (symbol === "mUSD") {
            apiLogger.info(`mUSD: Raw supply ${supply.toString()}, Decimals ${decimals}, Formatted ${(supply / divisor).toString()}`);
          }
          if (symbol === "MOD") {
            apiLogger.info(`MOD: Raw supply ${supply.toString()}, Decimals ${decimals}, Formatted ${(supply / divisor).toString()}`);
          }
    ...
    const coinSupplyPromises = bridgedCoins.map(async (coin) => {
      try {
        ...
        if (response.ok) {
          ...
          apiLogger.info(`Response for ${coin.symbol}: ${JSON.stringify(data, null, 2)}`);
          ...
          if (
            ...
            try {
              apiLogger.info("MOD supply vector is empty, trying GraphQL manual aggregation with pagination...");
              let totalSupply = BigInt(0);
              let offset = 0;
              ...
              while (hasMore) {
                ...
                if (graphqlResponse.ok) {
                  ...
                  if (graphqlResult.data?.current_coin_balances) {
                    ...
                    apiLogger.info(`Batch ${Math.floor(offset / batchSize) + 1}: Found ${balances.length} MOD holders`);
                  ...
                  apiLogger.error(`GraphQL request failed: ${graphqlResponse.status}`);
                  hasMore = false;
                }
              }

              supply = totalSupply;
              apiLogger.info(`MOD total supply from paginated GraphQL aggregation: ${supply.toString()}`);
            } catch (e) {
              apiLogger.error(`Failed to fetch MOD supply via GraphQL: ${e instanceof Error ? e.message : String(e)}`);
            }
          }

          // Log MOD specifically for debugging
          if (coin.symbol === "MOD") {
            apiLogger.info(`MOD final supply: ${supply.toString()} (should be ~500k = 50000000000000 raw with 8 decimals)`);
          }
          apiLogger.info(`${coin.symbol} supply: ${supply.toString()}`);
          ...
          apiLogger.error(`Failed to fetch ${coin.symbol}: HTTP ${response.status}`);
        }
        return { coin, supply: BigInt(0) };
      } catch (e) {
        apiLogger.error(`Failed to fetch supply for ${coin.symbol}: ${e instanceof Error ? e.message : String(e)}`);
        return { coin, supply: BigInt(0) };
      }
    });
    ...
    apiLogger.info(`Total supply (normalized to 6 decimals): ${totalSupply.toString()}`);
    ...
    return NextResponse.json(
      ...
      {
        headers: {
          // No cache - always fetch fresh stablecoin data
    ...
    apiLogger.error(`Error fetching stablecoin data: ${error instanceof Error ? error.message : String(error)}`);
    return NextResponse.json(
      {
