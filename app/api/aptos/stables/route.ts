...
export async function GET() {
  try {
    ...
    apiLogger.debug("Querying fungible assets:", fungibleAssets);
    ...
    if (result.errors) {
      apiLogger.error("GraphQL errors:", result.errors);
    ...
    const coinSupplyPromises = bridgedCoins.map(async (coin) => {
      try {
        ...
        if (response.ok) {
          ...
          apiLogger.debug(`Response for ${coin.symbol}:`, JSON.stringify(data, null, 2));
          ...
          if (
            coin.symbol === "MOD" &&
            (supply === BigInt(0) || (data.data?.supply?.vec && data.data.supply.vec.length === 0))
          ) {
            try {
              apiLogger.debug(
              ...
              apiLogger.error("Failed to fetch MOD supply via GraphQL:", e);
            }
          }
          ...
          apiLogger.debug(`${coin.symbol} supply:`, supply.toString());
        ...
        apiLogger.error(`Failed to fetch supply for ${coin.symbol}:`, e);
        return { coin, supply: BigInt(0) };
      }
    });
    ...
    apiLogger.debug("Total supply (normalized to 6 decimals):", totalSupply.toString());
    ...
    for (const item of supplies) {
      ...
      // Debug log for MOD
      if (item.symbol === "MOD") {
        apiLogger.debug(`${item.symbol} percentage calc:`, {
          supply: item.supply,
    ...
    apiLogger.error("Error fetching stablecoin data:", error);
    return NextResponse.json(
      {
