...
function SortableHeader({ column, title }: { column: unknown; title: string }) {
  return (
    <Button
      variant="ghost"
...
export default function TokensPage() {
  ...
  const [btcData, setBtcData] = useState<Record<string, unknown> | null>(null);
  ...
  const fetchBitcoinPrice = async () => {
    try {
      const response = await fetch("/api/prices/current?tokens=btc&source=cmc");
      if (response.ok) {
        const result = await response.json();
      ...
      logger.warn("Failed to fetch Bitcoin price");
    }
  };
  ...
  //     }
  //   } catch {
  //     logger.error("Failed to load remaining tokens");
  ...
  const getAdjustedPrice = useCallback((token: TokenData): number => {
  ...
  const displayMetrics = useMemo(() => {
    ...
    // Get verified and unverified counts from all tokens
    const verifiedCount = allTokens.filter((t) => t.isVerified === true).length;
    ...
    // Calculate stablecoins total market cap
    const stablecoins = allTokens.filter((token) => {
      const category = getTokenCategory(token);
      return category && category.label === "Stablecoin";
    });
    const stablecoinsMarketCap = stablecoins.reduce((sum, token) => {
      const adjustedPrice = getAdjustedPrice(token);
      return sum + adjustedPrice * (token.supply || 0);
    }, 0);
    ...
    // Calculate market caps with adjusted prices and sort for top tokens
    const tokensWithCaps = relevantTokens
      .map((token) => {
        const adjustedPrice = getAdjustedPrice(token);
        const marketCap = adjustedPrice * (token.supply || 0);
        return { ...token, calculatedMarketCap: marketCap };
      })
      .filter((t) => t.calculatedMarketCap > 0)
      .sort((a, b) => b.calculatedMarketCap - a.calculatedMarketCap);

    const tokenMarketCaps = tokensWithCaps.map((t) => t.calculatedMarketCap);
    const topTokens = tokensWithCaps.slice(0, 5);
    ...
    // Enhanced Distribution with all tiers
    const distribution = tokenMarketCaps.reduce(
      (dist, cap) => {
        ...
        else if (cap > 1000000) dist.from1MTo10M++;
        else if (cap > 100000) dist.from100KTo1M++;
        else dist.below100K++;
        dist.total++;
        return dist;
      },
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    ...
                    {!includeAPT &&
                      ...
                      `(${formatCurrency(displayMetrics.aptMarketCap, "USD", { compact: true })})`}
                  </label>
                </div>
                <div className="flex items-center gap-1">
                  <Checkbox
                  ...
                  />
                  <label
                    htmlFor="include-stables"
                    className="cursor-pointer select-none text-muted-foreground"
                  >
                    Stables{" "}
                    {!includeStables &&
                      displayMetrics.stablecoinsMarketCap > 0 &&
                      `(${formatCurrency(displayMetrics.stablecoinsMarketCap, "USD", { compact: true })})`}
                  </label>
                </div>
              </div>
            </div>
          }
          tooltip={`Total FDV: ${formatCurrency(displayMetrics.marketCap, "USD")}. APT: ${formatCurrency(displayMetrics.aptMarketCap, "USD")}, Stables: ${formatCurrency(displayMetrics.stablecoinsMarketCap, "USD")}`}
          isLoading={loading}
          showError={!!error}
        />
          ...
          value={
            <div className="space-y-1">
              <div className="text-lg md:text-2xl font-bold font-mono">
                {showUnverified
                  ? allTokens.length
                  : displayMetrics.verifiedCount}
              </div>
              <div className="text-xs text-muted-foreground">
                {showUnverified
                  ? `${displayMetrics.verifiedCount} verified • ${displayMetrics.unverifiedCount} unverified`
                  : `+${displayMetrics.unverifiedCount} unverified hidden`}
              </div>
            </div>
          }
          tooltip={`Total: ${allTokens.length} tokens (${displayMetrics.verifiedCount} verified, ${displayMetrics.unverifiedCount} unverified)`}
          isLoading={loading}
          showError={!!error}
        />
        ...
        <StatCard
          ...
          value={
            <div className="space-y-1">
              ...
              <div className="text-xs text-muted-foreground">
                Median:{" "}
                {formatCurrency((displayMetrics.medianMarketCap, "USD", {
                  compact: true,
                })}
              </div>
            </div>
          }
          tooltip={`Mean: ${formatCurrency(displayMetrics.averageMarketCap, "USD")}, Median: ${formatCurrency(displayMetrics.medianMarketCap, "USD")}`}
          isLoading={loading}
          showError={!!error}
        />

        <Card className="relative flex flex-col bg-card border rounded-xl p-4 md:p-5 shadow-sm hover:shadow-md transition-all duration-200 min-h-[100px] md:min-h-[120px]">
          <div className="mb-3">
          ...
          <div className="flex-1 flex flex-col justify-center space-y-2">
            ...
            <div className="flex items-end gap-1 h-12">
              {displayMetrics.distribution.above1B > 0 && (
                <div className="flex flex-col items-center flex-1">
                  <div
                    className="bg-green-500 dark:bg-green-400 rounded-t w-full transition-all duration-300 hover:opacity-80"
                    style={{
                      height: `${Math.max(20, (displayMetrics.distribution.above1B / Math.max(displayMetrics.distribution.above1B, displayMetrics.distribution.from100MTo1B, displayMetrics.distribution.from10MTo100M)) * 100)}%`,
                      minHeight: "20px",
                    }}
                    title={`${displayMetrics.distribution.above1B} tokens`}
                  >
                    <div className="text-[10px] font-bold text-white dark:text-black text-center pt-1">
              ...
              {displayMetrics.distribution.from100MTo1B > 0 && (
                <div className="flex flex-col items-center flex-1">
                  <div
                    className="bg-blue-500 dark:bg-blue-400 rounded-t w-full transition-all duration-300 hover:opacity-80"
                    style={{
                      height: `${Math.max(20, (displayMetrics.distribution.from100MTo1B / Math.max(displayMetrics.distribution.above1B, displayMetrics.distribution.from100MTo1B, displayMetrics.distribution.from10MTo100M)) * 100)}%`,
                      minHeight: "20px",
                    }}
                    title={`${displayMetrics.distribution.from100MTo1B} tokens`}
                  >
                    <div className="text-[10px] font-bold text-white dark:text-black text-center pt-1">
              ...
              {displayMetrics.distribution.from10MTo100M > 0 && (
                <div className="flex flex-col items-center flex-1">
                  <div
                    className="bg-purple-500 dark:bg-purple-400 rounded-t w-full transition-all duration-300 hover:opacity-80"
                    style={{
                      height: `${Math.max(20, (displayMetrics.distribution.from10MTo100M / Math.max(displayMetrics.distribution.above1B, displayMetrics.distribution.from100MTo1B, displayMetrics.distribution.from10MTo100M)) * 100)}%`,
                      minHeight: "20px",
                    }}
                    title={`${displayMetrics.distribution.from10MTo100M} tokens`}
                  >
                    <div className="text-[10px] font-bold text-white dark:text-black text-center pt-1">
              ...
              {displayMetrics.distribution.from1MTo10M > 0 && (
                <div className="flex flex-col items-center flex-1">
                  <div
                    className="bg-orange-500 dark:bg-orange-400 rounded-t w-full transition-all duration-300 hover:opacity-80"
                    style={{
                      height: `${Math.max(20, (displayMetrics.distribution.from1MTo10M / Math.max(displayMetrics.distribution.above1B, displayMetrics.distribution.from100MTo1B, displayMetrics.distribution.from10MTo100M, displayMetrics.distribution.from1MTo10M)) * 100)}%`,
                      minHeight: "20px",
                    }}
                    title={`${displayMetrics.distribution.from1MTo10M} tokens`}
                  >
                    <div className="text-[10px] font-bold text-white dark:text-black text-center pt-1">
                      {displayMetrics.distribution.from1MTo10M}
                    </div>
                  </div>
                  <div className="text-[9px] text-muted-foreground mt-1">
                    $1M
              ...
              {displayMetrics.distribution.from100KTo1M +
                displayMetrics.distribution.below100K >
                0 && (
                <div className="flex flex-col items-center flex-1">
                  <div
                    className="bg-gray-500 dark:bg-gray-400 rounded-t w-full transition-all duration-300 hover:opacity-80"
                    style={{
                      height: `${Math.max(20, ((displayMetrics.distribution.from100KTo1M + displayMetrics.distribution.below100K) / Math.max(displayMetrics.distribution.above1B, displayMetrics.distribution.from100MTo1B, displayMetrics.distribution.from10MTo100M, displayMetrics.distribution.from1MTo10M, displayMetrics.distribution.from100KTo1M + displayMetrics.distribution.below100K)) * 100)}%`,
                      minHeight: "20px",
                    }}
                    title={`${displayMetrics.distribution.from100KTo1M + displayMetrics.distribution.below100K} tokens`}
                  >
                    <div className="text-[10px] font-bold text-white dark:text-black text-center pt-1">
                      {displayMetrics.distribution.from100KTo1M +
                        displayMetrics.distribution.below100K}
