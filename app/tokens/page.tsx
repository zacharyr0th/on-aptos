...
import { ArrowUpDown, Copy, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronDown, TrendingUp, DollarSign, BarChart3, PieChart, Search, Download, Filter, Sparkles, Zap, AlertCircle } from 'lucide-react'
...
import { formatNumber, formatCurrency, formatTokenPrice } from '@/lib/utils/format'
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
export default function TokensPage() {
  ...
  const getTokenCategory = (token: TokenData) => {
    ...
    if (token.panoraTags && token.panoraTags.length > 0) {
      ...
      const primaryTag = token.panoraTags.find(tag => 
        ['Verified', 'Native', 'Bridged', 'LP', 'Emojicoin', 'Meme'].includes(tag)
      ) || token.panoraTags[0]
      
      switch (primaryTag) {
        case 'Native':
          return { label: 'Native', className: 'bg-primary/10 text-primary border-primary/30', panoraTag: 'Native' }
        case 'Bridged':
          return { label: 'Bridged', className: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400 border-cyan-300 dark:border-cyan-800', panoraTag: 'Bridged' }
        case 'LP':
          return { label: 'LP', className: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 border-gray-300 dark:border-gray-800', panoraTag: 'LP' }
        case 'Emojicoin':
          return { label: 'Emojicoin', className: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-300 dark:border-yellow-800', panoraTag: 'Emojicoin' }
        case 'Meme':
          return { label: 'Meme', className: 'bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-400 border-pink-300 dark:border-pink-800', panoraTag: 'Meme' }
        case 'Verified':
          return { label: 'Verified', className: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 border-green-300 dark:border-green-800', panoraTag: 'Verified' }
        default:
          return { label: primaryTag, className: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 border-gray-300 dark:border-gray-800', panoraTag: primaryTag }
      }
    }
    ...
    if (['USDC', 'USDT', 'USDt', 'USDe', 'sUSDe', 'mUSD', 'USDA', 'MOD', 'lzUSDC', 'lzUSDT', 'whUSDC', 'whUSDT', 'ceUSDC', 'ceUSDT'].includes(symbol) ||
        name.includes('usd') || name.includes('dollar') || name.includes('stable')) {
      return { label: 'Stablecoin', className: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-300 dark:border-emerald-800', panoraTag: 'Native' }
    }
    
    // Check for Bitcoin variants
    if (symbol.includes('BTC') || symbol.includes('WBTC') || symbol === 'aBTC' || symbol === 'SBTC' || symbol === 'fiaBTC' ||
        name.includes('bitcoin')) {
      return { label: 'Bitcoin', className: 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400 border-orange-300 dark:border-orange-800', panoraTag: 'Native' }
    }
    
    // Check for liquid staking tokens
    if ((symbol.includes('APT') && (symbol.includes('st') || symbol.includes('am'))) ||
        name.includes('staked') || name.includes('liquid') || symbol === 'amAPT') {
      return { label: 'Liquid Staking', className: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 border-purple-300 dark:border-purple-800', panoraTag: 'Native' }
    }
    
    // Check for RWA tokens
    if (['ZUIT', 'USTB', 'MORE', 'USDM'].includes(symbol) || name.includes('treasury') || name.includes('rwa') || name.includes('real world')) {
      return { label: 'RWA', className: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 border-amber-300 dark:border-amber-800', panoraTag: 'Native' }
    }
    
    // Check for known DeFi tokens
    if (['AMI', 'RION', 'THL', 'ECHO', 'LSD', 'CELL', 'CAKE', 'GUI', 'ABEL'].includes(symbol)) {
      return { label: 'DeFi', className: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border-blue-300 dark:border-blue-800', panoraTag: 'Native' }
    }
    
    // Check for bridged tokens (common bridge patterns)
    if (symbol.includes('lz') || symbol.includes('wh') || symbol.includes('ce') ||
        name.includes('layerzero') || name.includes('wormhole') || name.includes('celer')) {
      return { label: 'Bridged', className: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400 border-cyan-300 dark:border-cyan-800', panoraTag: 'Bridged' }
    }
    ...
    if (name.includes('emoji') || symbol.match(/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]/u)) {
      return { label: 'Emojicoin', className: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-300 dark:border-yellow-800', panoraTag: 'Emojicoin' }
    }
    
    // Meme tokens
    if (name.includes('doge') || name.includes('shib') || name.includes('pepe') || name.includes('meme') ||
    ...
    return { label: 'Other', className: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 border-gray-300 dark:border-gray-800', panoraTag: 'Native' }
  }
  ...
  const categoryFilters = useMemo(() => {
    const categories = [
      ...
      { name: 'Stablecoin', description: 'USD-pegged stablecoins', icon: DollarSign },
      { name: 'Bitcoin', description: 'Bitcoin variants (BTC, WBTC, etc)', icon: TrendingUp },
      { name: 'Liquid Staking', description: 'Liquid staking tokens (LST)', icon: BarChart3 },
      { name: 'RWA', description: 'Real World Assets', icon: PieChart },
      { name: 'DeFi', description: 'DeFi protocol tokens', icon: Filter },
      { name: 'Bridged', description: 'Cross-chain bridged tokens', icon: Search },
      { name: 'LP', description: 'Liquidity pool tokens', icon: BarChart3 },
      { name: 'Emojicoin', description: 'Emojicoin.fun tokens', icon: Sparkles },
    ...
    return categories.map(cat => ({
      ...cat,
      count: cat.name === 'All' 
        ? allTokens.filter(t => !showUnverified ? t.isVerified === true : true).length
        : allTokens.filter(t => {
            const category = getTokenCategory(t)
            return category.label === cat.name && (!showUnverified ? t.isVerified === true : true)
          }).length
    }))
  ...
  const getAdjustedPrice = (token: TokenData): number => {
    ...
    if ((token.symbol === 'SBTC' || token.symbol === 'fiaBTC') && btcData && bitcoinPrice) {
      ...
      const btcToken = btcData.supplies?.find((supply: any) => 
  ...
  const filteredTokens = useMemo(() => {
    ...
    if (!showUnverified) {
      tokens = tokens.filter(token => token.isVerified === true)
    }
    ...
    return tokens
  }, [allTokens, includeAPT, includeStables, btcData, bitcoinPrice, selectedTags, showUnverified])

  // Calculate metrics based on APT and stablecoin toggles
  const displayMetrics = useMemo(() => {
    if (!data || !allTokens.length) return {
      marketCap: 0,
      tokenCount: 0,
      averageMarketCap: 0,
      medianMarketCap: 0,
    ...
    if (!showUnverified) {
      relevantTokens = relevantTokens.filter(token => token.isVerified === true)
    }
    
    // Calculate total non-APT market cap
    const nonAptTokens = relevantTokens.filter(token => token.symbol !== 'APT')
    const nonAptMarketCaps = nonAptTokens.map(token => {
    ...
    const nonAptNoStablesMarketCaps = nonAptNoStablesTokens.map(token => {
      const adjustedPrice = getAdjustedPrice(token)
      return adjustedPrice * (token.supply || 0)
    }).filter(cap => cap > 0)
    const nonAptNoStablesTotalMarketCap = nonAptNoStablesMarketCaps.reduce((sum, cap) => sum + cap, 0)
    ...
    const tokenMarketCaps = relevantTokens.map(token => {
      const adjustedPrice = getAdjustedPrice(token)
      return adjustedPrice * (token.supply || 0)
    }).filter(cap => cap > 0)
    ...
    const medianMarketCap = sortedCaps.length > 0 
  ...
  }, [data, includeAPT, includeStables, allTokens, btcData, bitcoinPrice, showUnverified])
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="font-mono text-sm">{formatNumber(supply)}</span>
        },
      },
      {
        accessorKey: 'price',
        ...
        cell: ({ row }) => {
          ...
          return <span className="font-mono text-sm">{formatTokenPrice(price)}</span>
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="font-mono text-sm">{formatCurrency(marketCap)}</span>
        },
      },
    ],
  ...
  const quickFilters = [
    ...
    { label: 'New Listings', filter: () => filteredTokens.slice(-20).reverse() },
    { label: '> $100M', filter: () => filteredTokens.filter(t => t.marketCap > 100000000) },
  ]
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6 mb-8">
          ...
          value={
            <div className="space-y-2">
              <div className="text-base sm:text-lg md:text-2xl font-bold font-mono">
                {formatCurrency(displayMetrics.marketCap, 'USD', { compact: true })}
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    <Checkbox 
                      ...
                      onCheckedChange={(checked) => setIncludeStables(!!checked)}
                      className="h-3 w-3"
                    />
                    <label 
          ...
          value={
            <div className="space-y-1">
              ...
              <div className="text-xs text-muted-foreground">
                {(() => {
                  const verifiedCount = allTokens.filter(t => t.isVerified).length
        ...
        <StatCard
          ...
          value={
            <div className="space-y-1">
              <div className="text-base sm:text-lg md:text-2xl font-bold font-mono">
                {formatCurrency(displayMetrics.averageMarketCap, 'USD', { compact: true })}
              </div>
              <div className="text-xs text-muted-foreground space-y-0.5">
                {displayMetrics.aptMarketCap > 0 && displayMetrics.nonAptTotalMarketCap > 0 && (
                  <>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-1">
                      <div className="flex items-center gap-1">
                        <span>APT:</span>
                        <span className="font-semibold">{formatCurrency(displayMetrics.aptMarketCap, 'USD', { compact: true })}</span>
                      </div>
                      <span className="hidden sm:inline text-muted-foreground/70">vs</span>
                      <div className="flex items-center gap-1">
                        <span className="sm:hidden">ALT:</span>
                        <span className="font-semibold">{formatCurrency(displayMetrics.nonAptTotalMarketCap, 'USD', { compact: true })}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] sm:text-xs">Ratio:</span>
                      <span className="font-semibold text-[10px] sm:text-xs">
                        {displayMetrics.nonAptTotalMarketCap > displayMetrics.aptMarketCap 
          ...
          value={
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span>&gt;$1B:</span>
                <span className="font-semibold">{displayMetrics.distribution.above1B}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span>$100M-$1B:</span>
                <span className="font-semibold">{displayMetrics.distribution.from100MTo1B}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span>$10M-$100M:</span>
                <span className="font-semibold">{displayMetrics.distribution.from10MTo100M}</span>
              </div>
            </div>
          }
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start">
          ...
          <Dialog open={exportDialogOpen} onOpenChange={setExportDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Choose your preferred export format. The data will include all {filteredTokens.length} tokens currently visible in the table.
                </DialogDescription>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-4 py-4">
                <Button
                  ...
                  onClick={() => {
                    const csv = [
                      ['Symbol', 'Name', 'Price', 'Market Cap', 'Supply', 'Category', 'Address'].join(','),
                ...
                >
                  <div className="rounded-lg bg-muted p-2">
                    <svg className="h-8 w-8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      <path d="M14 2V8H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      <text x="12" y="16" fontSize="6" fontFamily="monospace" textAnchor="middle" fill="currentColor">{`{}`}</text>
                    </svg>
                  </div>
                  <div className="text-center">
          ...
          <div className="hidden sm:flex items-center gap-2 flex-wrap">
            ...
            {categoryFilters.map((filter) => {
              ...
              return (
                <Button
                  ...
                  onClick={() => {
                    if (filter.name === 'All') {
                      ...
                      setSelectedTags(prev => prev.filter(t => t !== filter.name))
                    } else {
                      setSelectedTags(prev => prev.filter(t => t !== 'All').concat(filter.name))
                    }
                  }}
                ...
                >
                  ...
                  {filter.count > 0 && (
                    <Badge variant="secondary" className="ml-1.5 px-1 py-0 h-4 text-[10px]">
                      {filter.count}
                    </Badge>
                  )}
                </Button>
            ...
            <Button
              ...
              className="text-xs h-10"
              title={showUnverified ? "Showing all tokens" : "Showing only verified tokens"}
            >
                    ...
                    <TableRow>
                      <TableCell colSpan={columns.length} className="h-48 text-center">
                        <div className="flex flex-col items-center justify-center space-y-3">
                          ...
                          {(globalFilter || (selectedTags.length > 0 && !selectedTags.includes('All'))) && (
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => {
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:justify-between">
          <div className="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
            <span className="hidden sm:inline">Showing </span>{table.getState().pagination.pageIndex * table.getState().pagination.pageSize + 1}-
            {Math.min(
              (table.getState().pagination.pageIndex + 1) * table.getState().pagination.pageSize,
              filteredTokens.length
            )}{' '}
            <span className="hidden sm:inline">of </span><span className="sm:hidden">/</span>{filteredTokens.length} {showUnverified ? 'tokens' : 'verified tokens'}{!includeAPT || !includeStables ? ' (' + [!includeAPT && 'APT excluded', !includeStables && 'Stablecoins excluded'].filter(Boolean).join(', ') + ')' : ''}
            {!hasLoadedAll && allTokens.length < (data?.summary.total_tokens || 0) && (
              <span className="text-xs ml-2">({allTokens.length} loaded)</span>
            )}
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div className="text-xs sm:text-sm px-2">
              ...
              {Math.ceil(filteredTokens.length / table.getState().pagination.pageSize)}
            </div>
            <Button
              variant="outline"
              size="sm"
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => table.setPageIndex(Math.ceil(filteredTokens.length / table.getState().pagination.pageSize) - 1)}
              disabled={!table.getCanNextPage()}
              className="h-8 w-8 sm:h-9 sm:w-9 p-0"
            >
