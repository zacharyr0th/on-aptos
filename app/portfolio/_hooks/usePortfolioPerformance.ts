    ...
    const fetchPortfolioPerformance = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          logger.error(
            `Portfolio performance API error: ${errorData instanceof Error ? errorData.message : String(errorData)}`,
          );
          throw new Error(
            ...
            result.data?.length > 1
              ? result.data.every(
                  (item: Record<string, unknown>) =>
                    item.value === result.data[0].value,
                )
              : false,
        });
        ...
        logger.error(
          `Portfolio performance error: ${err instanceof Error ? err.message : String(err)}`,
        );
