...
import { APTChart } from "../shared/APTChart";
import {
  cleanProtocolName,
  getProtocolLogo,
  getDetailedProtocolInfo,
} from "../shared/PortfolioMetrics";
import { DeFiSummaryView, NFTSummaryView } from "../shared/SummaryViews";
import { WalletSummary } from "../shared/WalletSummary";
import { TransactionHistoryTable } from "../tables/TransactionHistoryTable";
import { YieldTable } from "../tables/YieldTable";

interface PortfolioMainContentProps {
  activeTab: string;
  setActiveTab: (tab: string) => void;
  selectedAsset: unknown;
  handleAssetSelect: (asset: unknown) => void;
  selectedDeFiPosition: unknown;
  handleDeFiPositionSelect: (position: unknown) => void;
  selectedNFT: unknown;
  setSelectedNFT: (nft: unknown) => void;
  sidebarView: string;
  setSidebarView: (view: "assets" | "nfts" | "defi") => void;
  nfts: Array<Record<string, unknown>>;
  currentNFTs: Array<Record<string, unknown>>;
  ...
  pieChartColors: unknown;
  setHoveredCollection: (collection: unknown) => void;
...
export function PortfolioMainContent({
  ...
  const detailedProtocolInfo = selectedDeFiPosition
    ? getDetailedProtocolInfo(
        (selectedDeFiPosition as Record<string, unknown>).protocol as string,
      )
    : null;
    ...
    <div className="lg:col-span-3">
      ...
      <div>
        ...
        >
          <div className="w-full pb-3">
            <div className="flex justify-between items-center">
              <div className="flex justify-between items-end w-full border-b border-neutral-200 dark:border-neutral-800">
                <TabsList className="flex justify-start gap-4 lg:gap-8 rounded-none bg-transparent p-0 h-auto border-none overflow-x-auto">
                  <TabsTrigger
                    value="portfolio"
                    className="rounded-none px-0 pb-3 pt-0 h-auto data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-foreground border-b-2 border-transparent text-sm lg:text-base font-normal text-muted-foreground data-[state=active]:text-foreground whitespace-nowrap"
                  >
                    Portfolio
                  </TabsTrigger>
                  <TabsTrigger
                ...
                <div className="hidden lg:flex items-center pb-3">
                  <div className="text-right">
                    ...
                    <p className="text-lg font-geist-mono text-foreground">
                      <span className="font-bold">
                        {formatCurrency(
                          ((portfolioMetrics as Record<string, unknown>)
                            ?.totalPortfolioValue as number) || 0,
                        )}
                      </span>
                    </p>
                  </div>
            ...
            <div className="max-h-[calc(100vh-200px)] overflow-hidden">
              ...
              {selectedNFT ? (
                ( => {
                  ...
                  const sanitizedNFT = sanitizeNFTMetadata(
                    selectedNFT as Record<string, unknown>,
                  );
                  const nftRecord = selectedNFT as Record<string, unknown>;
                  const imageUrl = sanitizeImageUrl(
                    (nftRecord.cdn_image_uri as string) ||
                      (nftRecord.token_uri as string),
                    "/placeholder.jpg",
                  );
                                    ...
                                    {selectedNFT.cdn_image_uri && (
                                      <div>
                                        ...
                                        <button
                                          ...
                                          className="font-mono text-sm text-primary hover:underline break-all text-left"
                                        >
                                          {selectedNFT.cdn_image_uri}
                                        </button>
                                      </div>
                ...
                <>
                  {/* Portfolio Overview Card - Hidden when sidebarView is 'defi' or when any individual item is selected */}
                  {/* Also hidden on mobile to avoid duplication */}
                  {sidebarView !== "defi" &&
                    sidebarView !== "nfts" &&
                    !selectedAsset && (
                  ...
                  {/* NFT Summary View - Show when NFTs tab is selected on desktop */}
                  {sidebarView === "nfts" && !selectedNFT && (
                    <ScrollArea
              ...
              {selectedAsset && (
                ...
                >
                  ...
                  <div className="bg-card border rounded-lg p-4 flex flex-col">
                    <APTChart selectedAsset={selectedAsset} />
                  </div>
                            ...
                            <AccordionContent>
                              <ScrollArea className="h-[200px] pr-4">
                                <div className="space-y-3">
                                  ...
                                  {selectedAsset.price_24h_change !==
                                    undefined && (
                                    <div>
                                      <p className="text-sm text-muted-foreground">
                                        24h Change
                                      ...
                                      >
                                        {selectedAsset.price_24h_change >= 0
                                          ? "+"
                                          : ""}
                                        {selectedAsset.price_24h_change.toFixed(
                                          2,
                                        )}
                                        %
                                      </p>
              ...
              {sidebarView === "defi" && !selectedDeFiPosition && (
                <ScrollArea className="h-full" ref={defiSummaryRef}>
                  <DeFiSummaryView
          ...
          <TabsContent value="yield" className="mt-0 p-0">
