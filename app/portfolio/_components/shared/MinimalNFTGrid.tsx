        ...
        {filteredNFTs.map((item, _index) => {
          ...
          return (
            ...
            >
              ...
              {/* Subtle hover overlay - disabled on mobile for better performance */}
              <div className="absolute inset-0 bg-black/0 sm:group-hover:bg-black/10 transition-colors duration-200" />

              {/* Amount badge - only if more than 1 */}
              {(nft.amount ?? 1) > 1 && (
                <div className="absolute top-1.5 right-1.5 bg-white/90 dark:bg-black/90 backdrop-blur-sm px-1.5 py-0.5 text-xs rounded">
                  {nft.amount}
                </div>
              )}
            </div>
