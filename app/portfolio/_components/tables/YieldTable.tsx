...
// Map internal types to display labels and colors
...
export function YieldTable({ walletAddress }: YieldTableProps) {
  ...
  const columns = React.useMemo<ColumnDef<YieldOpportunity>[]>(
    () => [
      ...
      {
        id: "actions",
        header: () => <div className="text-center pl-4">Actions</div>,
        cell: ({ row }) => {
  ...
  // Calculate aggregate metrics - MUST be before any conditional returns for hooks rules
  const aggregateMetrics = React.useMemo(() => {
    const avgAPY =
      filteredOpportunities.length > 0
        ? filteredOpportunities.reduce((sum, opp) => sum + (opp.apy || 0), 0) /
          filteredOpportunities.length
        : 0;
    const highestAPY = Math.max(
      ...filteredOpportunities.map((opp) => opp.apy || 0),
      0,
    );
    const uniqueProtocolCount = new Set(
      filteredOpportunities.map((opp) => opp.protocol),
    ).size;

    // Get top performing protocol
    const topProtocol =
      filteredOpportunities.length > 0
    ...
    // Count by type
    const typeCount = filteredOpportunities.reduce(
      (acc, opp) => {
    ...
    return {
      ...
      uniqueProtocols: uniqueProtocolCount,
      topProtocol,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
          ...
          value={
            <div className="space-y-1">
              <div className="text-lg md:text-2xl font-bold font-mono">
                {aggregateMetrics.totalOpportunities}
              </div>
              <div className="text-xs text-muted-foreground">
                {aggregateMetrics.uniqueProtocols} protocol
                {aggregateMetrics.uniqueProtocols !== 1 ? "s" : ""}
              </div>
            </div>
          }
          tooltip={`${aggregateMetrics.totalOpportunities} yield opportunities across ${aggregateMetrics.uniqueProtocols} protocols`}
          isLoading={loading}
          showError={!!error}
        />
        ...
        <StatCard
          ...
          value={
            <div className="space-y-1">
              <div className="text-lg md:text-2xl font-bold font-mono">
                {formatPercentage(aggregateMetrics.avgAPY)}
              </div>
              <div className="text-xs text-muted-foreground">
                {aggregateMetrics.mostCommonType
                  ? `${typeConfig[aggregateMetrics.mostCommonType.toLowerCase()]?.label || aggregateMetrics.mostCommonType}`
                  : "Mixed"}{" "}
                focus
              </div>
            </div>
          }
          tooltip={`Average APY across all filtered opportunities`}
          isLoading={loading}
          showError={!!error}
        />
        ...
        <StatCard
          ...
          value={
            <div className="space-y-1">
              <div className="text-lg md:text-2xl font-bold font-mono">
                {formatPercentage(aggregateMetrics.highestAPY)}
              </div>
              <div className="text-xs text-muted-foreground truncate">
                {aggregateMetrics.topProtocol
                  ? aggregateMetrics.topProtocol.protocol
                  : "N/A"}
              </div>
            </div>
          }
          tooltip={`Highest APY opportunity: ${aggregateMetrics.topProtocol ? aggregateMetrics.topProtocol.protocol : "N/A"}`}
          isLoading={loading}
          showError={!!error}
        />
        ...
        <StatCard
          ...
          tooltip={`Number of active filters applied to the yield opportunities`}
          isLoading={loading}
          showError={!!error}
        />
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:grid sm:grid-cols-9 gap-2 sm:gap-4 w-full">
          ...
          <div className="grid grid-cols-4 sm:contents gap-2">
            <Button
              variant={selectedAsset === "APT" ? "default" : "outline"}
              size="sm"
              onClick={() =>
            ...
            <Button
              variant={selectedAsset === "SUSDE" ? "default" : "outline"}
              size="sm"
              onClick={() =>
            ...
            <Button
              variant={selectedAsset === "WBTC" ? "default" : "outline"}
              size="sm"
              onClick={() =>
