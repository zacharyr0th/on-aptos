...
export function formatCurrency(
  ...
  if (compact && Math.abs(amount) >= 1000) {
    ...
    const scaledNumber = amount / 1000 ** unitIndex;
 ...
 */
export function formatQuantityValue(value: number, symbol: string = ""): string {
 ...
 */
export function formatBigIntWithDecimals(value: bigint, decimals: number): string {
 ...
 */
export function convertRawTokenAmount(amount: string, decimals: number): number {
  const cleanAmount = amount.replace(/,/g, "");

  if (cleanAmount.includes(".")) {
    return parseFloat(cleanAmount);
  } else {
    return Number(BigInt(cleanAmount)) / 10 ** decimals;
  }
}
...
export function getDecimalPlaces(currencyCode: Currency | string): number {
  return isFiatCurrency(currencyCode.toUpperCase()) ? FIAT_DECIMALS : CRYPTO_DECIMALS;
}
