 ...
 * Main formatting functions for the application
 */
...
export function formatCompactNumber(value: number): string {
  ...
  try {
    ...
    errorLogger.warn({ error }, "Compact number formatting failed");
  ...
  try {
    ...
    if (typeof rawAmount === "string") {
      ...
      try {
        ...
        logger.warn({ rawAmount }, "Failed to parse rawAmount");
        return 0;
      }
    ...
    if (!Number.isFinite(correctDecimals) || correctDecimals < 0) {
      logger.warn({ correctDecimals }, "Invalid decimals");
...
export function formatTokenPrice(price: number | string): string {
  ...
  // For very small prices < $0.00001, use subscript notation
  if (numPrice < 0.00001) {
    const priceStr = numPrice.toFixed(20); // Get enough precision
    const match = priceStr.match(/^0\.0+/);
    if (match) {
      const zeroCount = match[0].length - 2; // Subtract "0."
      const significantPart = priceStr.replace(/^0\.0+/, "").replace(/0+$/, "");
      // Use subscript numbers for the zero count
      ...
      const subscriptZeros = zeroCount
        ...
        .join("");
      return `$0.0₍${subscriptZeros}₎${significantPart.substring(0, 4)}`;
    }
  }

  // For prices < $0.01, show up to 6 decimal places
  if (numPrice < 0.01) {
  ...
  // For prices >= $10,000, show no decimal places with thousands separator
  if (numPrice >= 10000) {
    return `$${Math.round(numPrice).toLocaleString("en-US")}`;
  }

  // For prices >= $100, show no decimal places
  if (numPrice >= 100) {
...
/** @deprecated Use formatBigIntWithDecimals directly */
export { formatBigIntWithDecimals as formatCurrencyBigInt };
/** @deprecated Use formatBigIntWithDecimals directly */
export { formatBigIntWithDecimals as formatNumberBigInt };

// Backward compatibility aliases
/** @deprecated Use formatPercentage directly */
export { formatPercentage as formatPercent };
