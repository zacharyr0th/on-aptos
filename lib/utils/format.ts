...
export function formatTokenPrice(
  ...
  if (price < 0.00001) {
    ...
    if (parts.length === 2) {
      ...
      const significantDigits = significantPart.substring(0, 2).replace(/0+$/, '') || significantPart.substring(0, 1);
      
      if (leadingZeros > 2 && significantDigits) { // Only use this format for more than 2 leading zeros
  ...
  return showSymbol ? formatCurrency(price) : formatCurrency(price).replace('$', '');
}
