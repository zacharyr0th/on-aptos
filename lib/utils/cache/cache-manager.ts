...
export const cacheInstances = {
  ...
  portfolio: new EnhancedLRUCache(
    SERVICE_CONFIG.portfolio.maxSize,
    SERVICE_CONFIG.portfolio.ttl,
  ),
} as const;
...
export async function cacheFirst<T>(options: CacheFirstOptions<T>) {
  ...
  if (!forceRefresh) {
    ...
    if (cachedData !== null) {
      ...
      const { buildCachedResponse } = await import("../api/response-builder");
  ...
  try {
    ...
    const { buildFreshResponse } = await import("../api/response-builder");
...
export async function cacheFirstWithFallback<T>(
  ...
  try {
    ...
    const { buildFallbackResponse } = await import("../api/response-builder");
