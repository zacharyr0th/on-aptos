...
import type { CacheEntry, CacheStats } from "../core/types";
...
export const cacheInstances = {
  ...
  portfolio: new EnhancedLRUCache(SERVICE_CONFIG.portfolio.maxSize, SERVICE_CONFIG.portfolio.ttl),
} as const;
...
export async function cacheFirstWithFallback<T>(options: CacheFirstOptions<T>, fallbackData: T) {
  try {
    ...
    logger.error(`Failed to fetch data for ${options.cacheKey}, using fallback:`, error);
