...
import { dedupeFetch } from "./request-deduplication";
import {
  FetchOptions,
  GraphQLRequest,
  BatchRequestOptions,
...
export async function apiRequest<T>(
  ...
  try {
    ...
    errorLogger.error(`${context} request error: ${error instanceof Error ? error.message : String(error)}`);
    throw new ApiError(
...
export async function batchRequests<T>(
  ...
  for (let i = 0; i < requests.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(
      batch.map((request) =>
        request().catch((error) => {
          errorLogger.error(`Batch request failed: ${error instanceof Error ? error.message : String(error)}`);
