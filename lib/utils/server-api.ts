...
import { RateLimitInfo, ApiResponse } from "./types";
...
function scheduleRateLimitCleanup(): void {
  setTimeout(() => {
    try {
      ...
      errorLogger.error(`Rate limit cleanup error occurred: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
...
export async function withApiEnhancements<T>(
  ...
  try {
    ...
    errorLogger.error(`API request failed: ${error instanceof Error ? error.message : String(error)}`);
