...
export class EnhancedTransactionAnalyzer {
  ...
  static analyzeTransaction(tx: Transaction): EnhancedTransactionInfo {
    ...
    const analyzed = this.analyzeProtocolInteraction(tx, analysis) ||
   ...
   */
  private static analyzeProtocolInteraction(tx: Transaction, analysis: EnhancedTransactionInfo): boolean {
   ...
   */
  private static analyzeLiquidStaking(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
   ...
   */
  private static analyzeLending(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
    analysis.category = TransactionCategory.DEFI;
   ...
   */
  private static analyzeDEX(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
    analysis.category = TransactionCategory.DEFI;
   ...
   */
  private static analyzeFarming(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
    analysis.category = TransactionCategory.DEFI;
   ...
   */
  private static analyzeBridge(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
    analysis.category = TransactionCategory.BRIDGE;
    analysis.subcategory = protocol.name.toLowerCase();
    analysis.activityType = analysis.direction === 'incoming' ? ActivityType.BRIDGE_IN : ActivityType.BRIDGE_OUT;
   ...
   */
  private static analyzeDerivatives(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
    analysis.category = TransactionCategory.DEFI;
   ...
   */
  private static analyzeNFTMarketplace(tx: Transaction, analysis: EnhancedTransactionInfo, protocol: ProtocolInfo): boolean {
   ...
   */
  private static analyzeCEXInteraction(tx: Transaction, analysis: EnhancedTransactionInfo): boolean {
   ...
   */
  private static analyzeBridgeActivity(tx: Transaction, analysis: EnhancedTransactionInfo): boolean {
    if (!tx.type) return false;
   ...
   */
  private static analyzeSystemActivity(tx: Transaction, analysis: EnhancedTransactionInfo): boolean {
    if (!tx.type) return false;
  ...
  private static analyzeAsset(assetType: string): EnhancedTransactionInfo['assetInfo'] {
    ...
    const isLST = Object.values(LIQUID_STAKING_TOKENS).includes(assetType as any);
    const isRWA = !!RWA_TOKEN_BY_ADDRESS[assetType];
    
    let displaySymbol = 'APT';
    
    if (isStablecoin) {
      displaySymbol = Object.entries(STABLECOINS).find(([_, addr]) => addr === assetType)?.[0] || 'Stablecoin';
    } else if (isLST) {
      displaySymbol = Object.entries(LIQUID_STAKING_TOKENS).find(([_, addr]) => addr === assetType)?.[0] || 'LST';
    } else if (isRWA) {
  ...
  private static adjustConfidence(tx: Transaction, analysis: EnhancedTransactionInfo): void {
    ...
    if (analysis.assetInfo?.isStablecoin || analysis.assetInfo?.isLST || analysis.assetInfo?.isRWA) {
