 ...
 */
export function buildCachedResponse<T>(data: T, startTime: number): ReturnType<typeof buildTRPCResponse<T>> {
...
export function buildFallbackResponse<T>(data: T, startTime: number): ReturnType<typeof buildTRPCResponse<T>> {
  return buildTRPCResponse(data, {
    startTime,
    cacheHits: 0,
    cacheMisses: 1,
...
/**
 * Create an error response with standard structure
 */
