...
export async function cacheFirst<T>(options: CacheFirstOptions<T>) {
  ...
  if (!forceRefresh) {
    ...
    if (cachedData !== null) {
      ...
      const { buildCachedResponse } = await import("./response-builder");
  ...
  try {
    ...
    const { buildFreshResponse } = await import("./response-builder");
...
export async function cacheFirstWithFallback<T>(
  ...
  try {
    ...
    const { buildFallbackResponse } = await import("./response-builder");
