...
/**
 * Stables specific schemas
 */
const StableSupplySchema = z.object({
  symbol: z.string(),
  supply: z.string(),
  supply_raw: z.string(),
  percentage: z.number(),
...
const StablesResponseSchema = BaseResponseSchema.extend({
  data: z.object({
...
// GraphQL query for fetching specific account balance
const BALANCE_QUERY = `
...
interface StablesGraphQLResponse {
...
// GraphQL response type for balance query
interface BalanceGraphQLResponse {
...
/**
 * Core data fetching logic for all supplies
 */
async function fetchAllSuppliesData(): Promise<Record<string, bigint>> {
...
/**
 * Core data fetching logic for Tether reserve balance
 */
async function fetchTetherReserveBalanceData(): Promise<bigint> {
  const fetchOptions: FetchOptions = {
    timeout: config.timeout,
    retries: config.retries,
    headers: {
...
/**
 * Core data processing logic for stablecoins supplies
 */
async function fetchStablesSuppliesData(): Promise<
  z.infer<typeof StablesResponseSchema>['data']
> {
      ...
      return await withErrorHandling(
        () =>
          cacheFirst({
            namespace: 'stables',
            cacheKey,
            fetchFn: fetchStablesSuppliesData,
            startTime,
