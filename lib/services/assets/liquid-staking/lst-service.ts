...
import { logger, serviceLogger, errorLogger } from '@/lib/utils/logger';
...
async function executeGraphQLQuery(
  ...
  serviceLogger.debug('[LST] Executing GraphQL query', {
...
function combineTokenSupplies(rawSupplies: LSTSupply[]): LSTSupply[] {
  ...
  for (const supply of rawSupplies) {
    ...
    if (combined[baseSymbol]) {
      ...
      serviceLogger.debug('[LST] Combined token supplies', {
        baseSymbol,
...
export async function fetchLSTSuppliesData(): Promise<LSTSupplyData> {
  ...
  serviceLogger.debug('[LST] Starting LST supplies fetch', {
  ...
  const requestFunctions = TOKENS.map(token => async () => {
    try {
      serviceLogger.debug('[LST] Fetching token supply', {
        symbol: token.symbol,
      ...
      serviceLogger.debug('[LST] Token type determined', {
        symbol: token.symbol,
        isFAOnlyToken,
        isCoinToken,
        assetType: token.asset_type,
      ...
      serviceLogger.debug('[LST] Raw GraphQL response for', token.symbol, {
        hasData: !!rawResult,
      ...
      if (isCoinToken) {
        ...
        if (coinInfo) {
          serviceLogger.debug('[LST] Coin info found', {
            symbol: token.symbol,
          ...
          try {
            ...
            serviceLogger.debug('[LST] Fetching coin supply via view function', {
              symbol: token.symbol,
              coinType: coinInfo.coin_type,
              viewUrl,
            });
            ...
            serviceLogger.debug('[LST] View function response', {
              symbol: token.symbol,
            ...
            if (
              ...
              logger.warn('[LST] Invalid view function response structure', {
                symbol: token.symbol,
                supplyData,
              });
            }
          ...
          logger.warn('[LST] No coin info found', {
            symbol: token.symbol,
        ...
        if (faMetadata && faMetadata.supply_v2) {
          ...
          serviceLogger.debug('[LST] FA metadata found', {
            symbol: token.symbol,
          ...
          logger.warn('[LST] No FA metadata found or no supply', {
            symbol: token.symbol,
      ...
      if (rawResult && typeof rawResult === 'object' && 'errors' in rawResult) {
        errorLogger.error('[LST] GraphQL errors received', {
          symbol: token.symbol,
      ...
      serviceLogger.debug('[LST] Token supply fetched successfully', {
        symbol: token.symbol,
        supply: totalAmount.toString(),
      ...
      errorLogger.error(`[LST] Failed to resolve token ${token.symbol}:`, {
        error: error.message,
        stack: error.stack,
        name: error.name,
      });
      ...
      if (
        ...
        errorLogger.error('[LST] Rate limit error detected', {
          symbol: token.symbol,
  ...
  serviceLogger.debug('[LST] Batch requests completed', {
  ...
  if (rateLimitError && successfulFetches === 0) {
    errorLogger.error('[LST] All requests failed due to rate limiting');
  ...
  serviceLogger.debug('[LST] LST supplies processing complete', {
  ...
  // Debug summary
  serviceLogger.debug('[LST] === FINAL DEBUG SUMMARY ===');
  serviceLogger.debug(
    `[LST] ✅ Successfully fetched ${successfulFetches}/${TOKENS.length} tokens`
  );
  serviceLogger.debug(`[LST] 💰 Total LST Value: ${totalFormatted}`);
  serviceLogger.debug('[LST] 🔢 Non-zero supplies:');
  supplies.forEach(s => {
    if (s.supply !== '0') {
      serviceLogger.debug(`[LST]   - ${s.symbol}: ${s.formatted_supply} (${s.supply})`);
    } else {
      serviceLogger.debug(`[LST]   - ${s.symbol}: ❌ ZERO SUPPLY`);
    }
  });
  serviceLogger.debug('[LST] === END DEBUG SUMMARY ===');
