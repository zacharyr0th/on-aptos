 ...
 */
export function groupDeFiByProtocol(positions: DeFiPosition[]): DeFiGroup[] {
...
export function calculateTotalAPY(defiGroups: DeFiGroup[]): number {
  ...
  defiGroups.forEach((group) => {
    group.positions.forEach((position) => {
      // Get APY from position assets or use 0
      let positionAPY = 0;
      if (position.position?.supplied) {
        positionAPY = position.position.supplied[0]?.apy || 0;
      } else if (position.position?.staked) {
        positionAPY = position.position.staked[0]?.apy || 0;
      } else if (position.position?.liquidity) {
        positionAPY = position.position.liquidity[0]?.apy || 0;
      }

      const value =
        position.totalValueUSD || position.totalValue || position.tvl_usd || 0;
      totalWeightedAPY += positionAPY * value;
      totalValue += value;
    });
  });
