...
import type { DeFiPosition as LegacyDeFiPosition } from "../types";
...
export class DeFiPositionConverter {
  ...
  static fromNewDeFiPosition(position: NewDeFiPosition): UnifiedDeFiPosition {
    ...
    return {
      ...
      protocolType: PROTOCOL_TYPE_MAPPING[position.protocolType] || position.protocolType,
      poolName: position.protocol,
      positionType: POSITION_TYPE_MAPPING[position.positionType] || position.positionType,
      suppliedAssets,
      borrowedAssets,
      liquidityAssets,
      stakedAssets,
      totalValueUSD: position.totalValueUSD,
      healthFactor: typeof position.metadata?.healthFactor === 'number' ? position.metadata.healthFactor : undefined,
  ...
  static validatePosition(position: UnifiedDeFiPosition): {
    ...
    if (!position.suppliedAssets && !position.borrowedAssets && 
        !position.liquidityAssets && !position.stakedAssets) {
      errors.push("Position must have at least one asset");
    }

    // Validate asset data
    [...position.suppliedAssets, ...position.borrowedAssets, ...position.stakedAssets]
      .forEach((asset, index) => {
        if (!asset.asset) {
