...
export class TokenRegistry {
  ...
  static getSymbolFromAddress(address: string): string {
    ...
    if (address.includes("::thl_coin::") || address.includes("::THL")) return "THL";
    if (address.includes("::mod_coin::") || address.includes("::MOD")) return "MOD";
  ...
  static isLiquidStakingToken(address: string): boolean {
    return Object.values(LIQUID_STAKING_ADDRESSES).includes(address as any) ||
           address.includes("::stapt_token::") ||
           address.includes("::StakedApt") ||
           address.includes("::amapt_token::") ||
           address.includes("::AmnisApt") ||
  ...
  static mapToUnderlyingAsset(address: string, symbol?: string): string | null {
    ...
    if (symbol?.toLowerCase().includes("apt") || this.isLiquidStakingToken(address)) {
  ...
  static isSuspiciousToken(address: string, symbol?: string): boolean {
    ...
    const scamPatterns = [
      /test/i, /fake/i, /scam/i, /airdrop/i,
      /reward/i, /bonus/i, /gift/i, /claim/i
    ];

    return scamPatterns.some(pattern => 
      pattern.test(address) || (symbol && pattern.test(symbol))
    );
  }
