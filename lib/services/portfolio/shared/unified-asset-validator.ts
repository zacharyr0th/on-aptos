...
export class UnifiedAssetValidator {
  ...
  static validateAsset(
    ...
    if (checkBalance && balance && this.hasSuspiciousBalance(balance, balanceThreshold)) {
  ...
  static isLegitimateToken(address: string, symbol?: string): boolean {
    ...
    if (TokenRegistry.isStablecoin(address, symbol) && 
        this.isLegitimateStablecoin(address)) return true;
   ...
   */
  static filterAssets<T extends { asset_type: string; metadata?: { symbol?: string }; balance?: number }>(
    assets: T[],
    criteria: {
      excludeScam?: boolean;
  ...
  private static isFakeToken(address: string, symbol?: string): boolean {
    ...
    if (symbol && TokenRegistry.isStablecoin(address, symbol) && 
