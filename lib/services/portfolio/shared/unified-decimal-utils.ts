...
export class UnifiedDecimalUtils {
  ...
  static formatBalance(
    ...
    const minDisplayThreshold = options.minDisplayThreshold || PORTFOLIO_THRESHOLDS.MIN_BALANCE_DISPLAY;
  ...
  static batchFormatBalances(
    ...
    return balances.map(({ amount, tokenAddress, symbol, price }) => {
      const formattedBalance = this.formatBalance(amount, tokenAddress, symbol, options);
      
      const result = { ...formattedBalance } as FormattedBalance & { value?: TokenValue };
      
      if (price !== undefined && price > 0) {
        result.value = this.calculateTokenValue(formattedBalance.formatted, price, options);
      }
  ...
  static parseUserInput(
    ...
    try {
      ...
      const decimals = tokenAddress ? TokenRegistry.getTokenDecimals(tokenAddress, symbol) : DECIMALS.DEFAULT;
