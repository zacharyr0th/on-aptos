...
export class UnifiedPriceService {
   ...
   */
  private static readonly HARDCODED_PRICES: Record<string, Omit<PriceData, 'source'>> = {
    // MKLP tokens
    "::house_lp::MKLP": {
      price: 1.05,
      change24h: 0,
  ...
  static async getAssetPrice(
    ...
    try {
      ...
      for (const [pattern, priceData] of Object.entries(this.HARDCODED_PRICES)) {
      ...
      try {
        ...
        logger.warn(
          `Panora price fetch failed for ${assetType}:`,
          panoraError,
        );
      }
  ...
  static async getBatchPrices(
    ...
    try {
      ...
      if (uncachedTypes.length > 0) {
        ...
        try {
          const panoraPrices = await PanoraService.getTokenPrices(uncachedTypes);
