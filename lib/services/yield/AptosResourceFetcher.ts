import { YIELD_PROTOCOL_ADDRESSES, YIELD_TOKEN_ADDRESSES, getSymbolFromAddress } from "@/lib/constants";
...
export class AptosResourceFetcher {
  ...
  private async parseResourceToOpportunity(
    ...
    try {
      const assets = config.opportunityType === "liquidity" 
  ...
  calculateTVL(resourceData: any, opportunityType: string): number {
    ...
    switch (opportunityType) {
      case "lending":
        return parseFloat(resourceData.underlying_coin?.value || resourceData.total_supplied || "0") / 1e8;
      case "liquidity":
        const reserve0 = parseFloat(resourceData.coin_x_reserve?.value || resourceData.asset_0?.value || "0");
        const reserve1 = parseFloat(resourceData.coin_y_reserve?.value || resourceData.asset_1?.value || "0");
        return (reserve0 + reserve1) / 1e8;
      case "staking":
        return parseFloat(resourceData.total_staked?.value || resourceData.pool_balance?.value || "0") / 1e8;
      default:
        return 0;
    }
  }
   ...
   */
  private extractRewards(resourceData: any): Array<{ token: string; symbol: string; apr: number }> | undefined {
    if (!resourceData?.reward_tokens && !resourceData?.rewards) return undefined;
    
    const rewardTokens = resourceData.reward_tokens || resourceData.rewards || [];
    return rewardTokens.map((token: any) => ({
      token: typeof token === 'string' ? token : token.token || token.address,
      symbol: this.getSymbol(typeof token === 'string' ? token : token.token || token.address),
  ...
  private isResourceActive(resourceData: any): boolean {
    ...
    const hasValue = parseFloat(resourceData.underlying_coin?.value || resourceData.total_supplied || "0") > 0;
    const isNotPaused = !resourceData.is_paused && !resourceData.paused;
    const hasPositiveAPY = parseFloat(resourceData.supply_apy || resourceData.apy || "0") > 0;
  ...
  private calculateLendingAPY(resourceData: any): number {
    const apy = resourceData.supply_apy || 
  ...
  private calculateLiquidityAPY(resourceData: any): number {
    ...
    const rewardAPY = resourceData.reward_apy || resourceData.incentive_apy || 0;
    return feeAPY + rewardAPY;
  }

  private calculateStakingAPY(resourceData: any): number {
    return resourceData.staking_apy || resourceData.validator_apy || resourceData.apy || 0;
  }
