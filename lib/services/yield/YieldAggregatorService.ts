...
import { DeFiPosition } from "@/lib/services/defi/types";
import { logger } from "@/lib/utils/logger";

import { AptosResourceFetcher, ProtocolOpportunity } from "./AptosResourceFetcher";
...
export class YieldAggregatorService {
  ...
  async discoverOpportunities(
    ...
    try {
      ...
      const [lending, liquidity, staking, farming, defiLlama] = await Promise.all([
      ...
      opportunities.push(...lending, ...liquidity, ...staking, ...farming, ...defiLlama);
  ...
  async generateStrategies(
    ...
    if (preferences.riskTolerance === "conservative") {
      ...
      strategies.push({
        ...
        risk: "conservative",
        protocols: [...new Set(conservativeOps.slice(0, 3).map(o => o.protocol))],
        allocation: this.calculateOptimalAllocation(conservativeOps.slice(0, 3)),
        estimatedGas: 0.5,
        steps: this.generateStrategySteps(conservativeOps.slice(0, 3), preferences.availableCapital),
      });
    }
    ...
    if (preferences.riskTolerance === "aggressive") {
      const aggressiveOps = opportunities
        .filter(o => o.apy > 10)
        .slice(0, 7);
    ...
    try {
      ...
      const [aries, echelon, echo, meso] = await Promise.allSettled([
        this.fetchProtocolOpportunities("Aries", PROTOCOL_ADDRESSES.ARIES, {
          resourceFilters: [`${PROTOCOL_ADDRESSES.ARIES}::reserve::ReserveCoinContainer`],
      ...
      const allResults = [aries, echelon, echo, meso].filter(result => result.status === 'fulfilled');
  ...
  private async fetchLiquidityOpportunities(): Promise<YieldOpportunity[]> {
    ...
    try {
      const [thala, liquidswap] = await Promise.allSettled([
        ...
        this.fetchProtocolOpportunities("LiquidSwap", PROTOCOL_ADDRESSES.LIQUIDSWAP, {
  ...
  private async fetchStakingOpportunities(): Promise<YieldOpportunity[]> {
    ...
    try {
      ...
      // Process results
      const allResults = [tortuga, amnis].filter(result => result.status === 'fulfilled');
      for (const result of allResults) {
        if (result.status === 'fulfilled') {
          opportunities.push(...result.value.map(this.convertProtocolToYieldOpportunity));
        }
  ...
  private async fetchFarmingOpportunities(): Promise<YieldOpportunity[]> {
    ...
    try {
      const [thalaFarm] = await Promise.allSettled([
        this.fetchProtocolOpportunities("Thala Farm", PROTOCOL_ADDRESSES.THALA_FARM, {
