...
export class AnsService {
  ...
  static async resolveName(domain: string): Promise<AnsResolveResult> {
    ...
    if (!cleanDomain.endsWith(".apt")) {
      ...
      return { address: null, domain: cleanDomain, subdomain: null, source: "ans-api" };
    }
    ...
    try {
      ...
      if (nameRecord?.registered_address) {
        ...
        if (nameRecord.expiration_timestamp) {
          ...
          const now = Date.now();
          if (expiration < now) {
            logger.info(`[ANS] Domain ${cleanDomain} has expired on ${nameRecord.expiration_timestamp}`);
            return { address: null, domain: domainName, subdomain, source: "graphql" };
          }
        }

        logger.info(`[ANS] Resolved ${cleanDomain} to ${nameRecord.registered_address} via GraphQL`);
        return {
