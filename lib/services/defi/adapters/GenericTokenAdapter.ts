import { PROTOCOLS, ProtocolInfo, ProtocolType as RegistryProtocolType } from "@/lib/protocol-registry";
...
export class GenericTokenAdapter extends BaseProtocolAdapter {
  ...
  // Map registry protocol types to DeFi protocol types
  private mapProtocolType(registryType: RegistryProtocolType): ProtocolType {
    const typeMapping: Record<RegistryProtocolType, ProtocolType> = {
      [RegistryProtocolType.LIQUID_STAKING]: ProtocolType.LIQUID_STAKING,
      [RegistryProtocolType.LENDING]: ProtocolType.LENDING,
      [RegistryProtocolType.BRIDGE]: ProtocolType.BRIDGE,
      [RegistryProtocolType.FARMING]: ProtocolType.FARMING,
      [RegistryProtocolType.DEX]: ProtocolType.DEX,
      [RegistryProtocolType.DERIVATIVES]: ProtocolType.DERIVATIVES,
      [RegistryProtocolType.INFRASTRUCTURE]: ProtocolType.UNKNOWN,
      [RegistryProtocolType.NFT_MARKETPLACE]: ProtocolType.NFT,
    };
    return typeMapping[registryType] || ProtocolType.UNKNOWN;
  }
  ...
  private identifyProtocolFromAddress(
    ...
    for (const [address, protocol] of this.protocolRegistry.entries()) {
      if (tokenAddress.toLowerCase().includes(address.toLowerCase())) {
        return {
          name: protocol.name,
          type: this.mapProtocolType(protocol.type),
        };
      }
    }
  ...
  private isCommonToken(tokenAddress: string): boolean {
    ...
    return (
      commonTokens.some((token) => tokenAddress.includes(token)) ||
  ...
  isSupported(_protocolAddress: string): boolean {
