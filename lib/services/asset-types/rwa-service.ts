...
import { CACHE_TTL, API_ENDPOINTS } from "@/lib/constants";
import { enhancedFetch } from "@/lib/utils/fetch-utils";
import { logger } from "@/lib/utils/logger";

import type { RWAAsset } from "../shared/types";
import { BaseAssetService } from "../shared/utils/base-service";
import { sortByStringSupply } from "../shared/utils/formatting";
...
export class RWAService extends BaseAssetService {
  ...
  private static processRWAAssets(assets: RWATokenResponse[]): RWAAsset[] {
    return sortByStringSupply(
      assets.map((asset) => ({
        asset: asset.id,
        name: asset.name,
        symbol: asset.symbol,
        supply: asset.total_supply.toString(),
