...
export class StablecoinService extends BaseAssetService {
    ...
    if (result.fungible_asset_metadata) {
      for (const item of result.fungible_asset_metadata) {
        if (item.supply_v2) {
          ...
          let supply = BigInt(item.supply_v2?.vec?.[0] || "0");
          ...
          if (symbol === "USDT") {
            ...
            logger.info(
              `USDT: Total supply ${item.supply_v2?.vec?.[0] || "0"}, Reserve ${usdtReserveBalance}, Circulating ${supply.toString()}`,
            );
          }
          ...
          supplies.push({
            ...
            issuer: "Unknown", // Default value, metadata doesn't contain this
            peggedTo: "USD", // Assuming most are USD-pegged
            isAlgorithmic: false, // Default assumption
  ...
  private static async fetchBridgedCoinSupplies(): Promise<StablecoinSupply[]> {
    const supplies = await Promise.all(
      BRIDGED_COINS.map((_coin) => this.fetchCoinSupply(_coin)),
    );
