...
export class BitcoinService extends BaseAssetService {
  ...
  private static async fetchTokenSupply(
    ...
    try {
      ...
      switch (token.type) {
        ...
        case "FA_aggregate":
          ...
          if (token.symbol === "aBTC") {
            ...
            try {
              ...
              if (data.errors) {
                throw new Error(
                  `GraphQL errors: ${data.errors.map((e: Record<string, unknown>) => e.message).join(", ")}`,
                );
              }
        ...
        case "coin_v1":
          // For coin v1 tokens, try to get metadata from fungible_asset_metadata
          try {
            ...
            if (metadata?.supply_v2?.vec?.[0]) {
              supply = metadata.supply_v2.vec[0];
            } else {
              // Fall back to REST API approach for coin v1 tokens
              throw new Error(
                "No supply info in fungible_asset_metadata, falling back to REST API",
              );
            }
          } catch (error) {
            // Fall back to REST API approach for coin v1 tokens
   ...
   */
  private static extractCoinInfoSupply(data: Record<string, unknown>): string {
