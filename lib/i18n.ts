...
if (isServer) {
  ...
  if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
    try {
      ...
      logger.warn(`Failed to apply saved language preference: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export const preloadNamespace = async (namespace: Namespace): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`Failed to preload namespace ${namespace}: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
export const preloadPageTranslations = async (pageNamespaces: Namespace[]): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`Failed to preload page translations: ${error instanceof Error ? error.message : String(error)}`);
  }
};
