...
if (isServer) {
  ...
  if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
    try {
      ...
      logger.warn("Failed to apply saved language preference:", error);
    }
  }
}
...
export const initializeI18n = async (): Promise<boolean> => {
  ...
  logger.info("i18n already initialized", {
...
export const preloadNamespace = async (namespace: Namespace): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`Failed to preload namespace ${namespace}`, error);
  }
};
...
export const preloadPageTranslations = async (pageNamespaces: Namespace[]): Promise<void> => {
  ...
  try {
    ...
    logger.warn("Failed to preload page translations", error);
  }
};
