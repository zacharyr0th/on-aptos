  ...
  try {
      ...
      positions.forEach((position, index) => {
        ...
        if (pos.liquidity && pos.liquidity.length > 0) {
          ...
          pos.liquidity.forEach((liq, i) => {
            console.log(`     ${i + 1}. ${liq.token0.symbol}/${liq.token1.symbol} (${liq.poolId})`);
    ...
    if (stats.topProtocols.length > 0) {
      ...
      stats.topProtocols.forEach((protocol, index) => {
        console.log(`${index + 1}. ${protocol.protocol}: $${protocol.value.toFixed(2)} (${protocol.percentage.toFixed(1)}%)`);
      });
    }
