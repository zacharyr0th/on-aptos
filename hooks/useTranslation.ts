    ...
    const loadNamespaces = async () => {
      if (typeof window !== "undefined" && isMounted) {
        try {
          ...
          logger.warn(`Failed to load namespaces: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const t = useCallback(
    (key: string, fallback?: string, options?: any): string => {
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          logger.warn(`Translation error for key "${key}": ${error instanceof Error ? error.message : String(error)}`);
        }
        return fallback || key;
      }
    },
  ...
  const changeLanguage = useCallback(
    async (lng: SupportedLanguage): Promise<void> => {
      try {
        ...
        errorLogger.error(`Failed to change language: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
    [i18n],
  );
