...
const protocolMetricsCache = new Map<string, Promise<Record<string, unknown>>>();
              ...
              return {
                ...
                yields:
                  metrics.yields && metrics.yields.length > 0
                    ? {
                        ...protocol.yields,
                        pools: metrics.yields.map((pool: Record<string, unknown>) => ({
                          poolId: pool.pool,
                          symbol: pool.symbol,
                          apy: pool.apy,
                        ...
                        max: metrics.yields
                          .reduce(
                            (max: number, pool: Record<string, unknown>) =>
                        ...
                        average: (
                          metrics.yields.reduce(
                            (sum: number, pool: Record<string, unknown>) => sum + (pool.apy || 0),
                            0,
                  ...
                  metrics.borrowRates || metrics.supplyRates
                    ? {
                        ...
                        totalBorrowed: metrics.borrowRates
                          ?.reduce(
                            (sum: number, r: Record<string, unknown>) => sum + r.totalBorrowUsd,
                            0,
                          )
                          .toString(),
                        totalSupplied: metrics.supplyRates
                          ?.reduce(
                            (sum: number, r: Record<string, unknown>) => sum + r.totalSupplyUsd,
                            0,
                          )
                          .toString(),
                      }
