  ...
  const { marketSharePercent, formattedDisplaySupply } = useMemo(() => {
    ...
    const formatSingle = (s: string, symbol: string, isRaw: boolean = true) => {
      ...
      if (isRaw) {
        ...
        const decimals = ["mUSD", "MOD", "USDA"].includes(symbol) ? 100_000_000 : 1_000_000;
...
export default function StablesPage(): React.ReactElement {
  ...
  const fetchStablesData = useCallback(async () => {
    try {
      ...
      logger.info(`[Stables Page] Fetched data: ${JSON.stringify(data).substring(0, 200)}`);
      setStablesData(data);
    } catch (error) {
      logger.error(`[Stables Page] Error fetching data: ${error instanceof Error ? error.message : String(error)}`);
      setStablesError(
  ...
  // Debug logging
  if (data) {
    logger.info(`[Stables Page] Data structure: has supplies = ${!!data.supplies}, supplies length = ${data.supplies?.length}`);
  }
                          ...
                          <TableRow key={token.symbol}>
                            <TableCell className="whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                <Image
                                  src={
                                    (metadata?.thumbnail as string) || "/placeholder.jpg"
                                  }
                                  alt={token.symbol}
                                  width={20}
                                  height={20}
