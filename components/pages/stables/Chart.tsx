...
const CustomTooltip = memo<TooltipProps<number, string>>((props) => {
  ...
  try {
    ...
    logger.error(`Error rendering tooltip: ${error instanceof Error ? error.message : String(error)}`);
    return (
      ...
      {remainingCount > 0 && (
        <div className="flex items-center gap-3">
          <div
            className="w-3 h-3 rounded-sm flex-shrink-0"
            style={{
              backgroundColor: "hsl(240 5% 84%)", // Gray color for "more" indicator
            }}
          />
...
export const MarketShareChart = memo<MarketShareChartProps>(
  ({
    ...
    const getCellColor = useCallback((entry: ChartDataItem) => {
      if (entry.name === "Other") {
        return "hsl(240 5% 45%)"; // Darker gray color for "Other" category
      }
