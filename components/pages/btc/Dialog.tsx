  ...
  const formattedData = useMemo(() => {
    return measurePerformance(() => {
      try {
        ...
        logger.error(`Error calculating BTC values: ${error instanceof Error ? error.message : String(error)}`);
        return {
          btcAmount: t(
  ...
  ({ isOpen, onClose, metadata, supply, bitcoinPrice = 0 }) => {
    ...
    const formattedSupply = useMemo(() => {
      ...
      return (
        <SupplyCalculator
          supply={supply}
          decimals={typeof metadata.decimals === 'number' ? metadata.decimals : 8}
    ...
    const formattedAddresses = useMemo(() => {
      ...
      return (
        <AddressDisplay
          address={typeof metadata.assetAddress === 'string' ? metadata.assetAddress : ''}
          onCopy={handleCopy}
          t={t}
        />
      );
    ...
    const headerContent = useMemo(() => {
      ...
      return (
        <div className="flex items-center gap-3">
          <TokenIcon src={typeof metadata.thumbnail === 'string' ? metadata.thumbnail : ''} alt={`${typeof metadata.name === 'string' ? metadata.name : ''} icon`} />
          <div>
            <DialogTitle className="text-xl">{typeof metadata.name === 'string' ? metadata.name : ''}</DialogTitle>
            <p className="text-sm text-muted-foreground">{typeof metadata.symbol === 'string' ? metadata.symbol : ''}</p>
          </div>
        </div>
      );
    }, [metadata]);
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center gap-2 p-3 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-950/30 dark:to-amber-950/30 rounded-lg border border-orange-200 dark:border-orange-800">
            <Bitcoin className="h-4 w-4 text-orange-500 dark:text-orange-400" />
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="space-y-1">
              ...
              <p className="font-mono">{typeof metadata.decimals === 'number' ? metadata.decimals : 8}</p>
            </div>
            <div className="space-y-1">
              <p className="text-muted-foreground">
