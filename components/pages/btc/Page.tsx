...
import { MarketShareChart } from "@/components/pages/btc/Chart";
...
const copyToClipboard = async (text: string, label: string = "Address"): Promise<void> => {
  try {
    ...
    <>
      ...
      {tokenData.metadata && (
        <TokenDialog
          ...
          symbol={typeof tokenData.metadata.symbol === 'string' ? tokenData.metadata.symbol : ''}
          name={typeof tokenData.metadata.name === 'string' ? tokenData.metadata.name : ''}
          decimals={typeof tokenData.metadata.decimals === 'number' ? tokenData.metadata.decimals : 8}
...
export default function BitcoinPage(): React.ReactElement {
  ...
  const fetchBtcSupplyData = useCallback(async () => {
    try {
      ...
      logger.error(`[BTC Page] Error fetching data: ${error instanceof Error ? error.message : String(error)}`);
      setBtcSupplyError(
  ...
  const isRateLimited = btcSupplyError?.message?.includes("Rate limited") || btcSupplyError?.message?.includes("429");
                    ...
                    <MarketShareChart
                      ...
                      tokenMetadata={Object.fromEntries(
                        Object.entries(TOKEN_METADATA)
                          .filter(([_, metadata]) => metadata.assetAddress)
                          .map(([key, metadata]) => [
                            key, 
                            {
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {processedData?.supplies?.map((token: any) => {
                        ...
                        const marketSharePercent = totalBTC > 0 ? ((btcAmount / totalBTC) * 100).toFixed(2) : "0.00";
                          ...
                          <TableRow key={token.symbol}>
                            ...
                            <TableCell className="whitespace-nowrap hidden sm:table-cell">
                              {metadata?.assetAddress ? (
                                <button
                                  onClick={() => copyToClipboard(metadata.assetAddress!, `${token.symbol} address`)}
