...
export function TransactionInsights({
  ...
  const [transactions, setTransactions] = useState<EnhancedTransaction[] | null>(
    null,
  );
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const enhancedTransactions: EnhancedTransaction[] = rawTransactions.map((tx: WalletTransaction) => ({
        ...tx,
  ...
  const getTransactionInsights = (txs: EnhancedTransaction[]) => {
    ...
    // Category breakdown
    const categories = txs.reduce((acc, tx) => {
    ...
    // Asset diversity
    const assets = new Set(txs.map(tx => 
    ...
    const getActivityLevel = (weeklyTxs: number) => {
      if (weeklyTxs >= 20)
        return { level: t("portfolio.transactions.very_active"), color: "text-green-600 dark:text-green-400", icon: Zap };
      if (weeklyTxs >= 10)
        return { level: t("portfolio.transactions.active"), color: "text-blue-600 dark:text-blue-400", icon: TrendingUp };
      if (weeklyTxs >= 5)
        return { level: t("portfolio.transactions.moderate"), color: "text-yellow-600 dark:text-yellow-400", icon: Activity };
      if (weeklyTxs >= 1)
        return { level: t("portfolio.transactions.light"), color: "text-orange-600 dark:text-orange-400", icon: Clock };
      return { level: t("portfolio.transactions.inactive"), color: "text-gray-600 dark:text-gray-400", icon: TrendingDown };
    };

    // DeFi/Staking engagement
    const defiTxs = Object.entries(categories)
      .filter(([cat]) => [TransactionCategory.DEFI, TransactionCategory.STAKING].includes(cat as TransactionCategory))
    ...
    return {
      ...
      protocols,
      categories,
      mostCommonCategory: mostCommonCategory ? mostCommonCategory[0] : TransactionCategory.UNKNOWN,
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground">{t("portfolio.transactions.total_transactions")}</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground">{t("portfolio.transactions.assets")}</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold">{Math.round(insights.defiEngagement * 100)}%</p>
            ...
            <div className="p-3 rounded border bg-muted/30">
              ...
              <div className="flex items-center gap-2 mt-1">
                {(() => {
                  const Icon = getCategoryIcon(insights.mostCommonCategory as TransactionCategory);
                  return <Icon className="h-4 w-4 text-muted-foreground" />;
                })()}
                <p className="text-sm font-medium">
                  {EnhancedTransactionAnalyzer.getCategoryDisplayName(insights.mostCommonCategory as TransactionCategory)}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-xs font-medium">{t("portfolio.transactions.recent_activity")}</p>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <ScrollArea className="h-48">
                <div className="space-y-2">
                  {transactions.slice(0, 8).map((tx, index) => {
                    ...
                    const displaySymbol = tx.analysis?.assetInfo?.displaySymbol || 
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {tx.analysis?.confidence && tx.analysis.confidence >= 80 && (
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {tx.success ? t("portfolio.transactions.success") : t("portfolio.transactions.failed")}
                          </p>
                        </div>
                      </div>
                    );
            ...
            <div className="pt-2 border-t border-border/50">
              <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
                ...
                <span>
                  {t("portfolio.transactions.transactions_in_last_days", { count: insights.recentTxs, days: 7 })}
                </span>
              </div>
            </div>
          </>
