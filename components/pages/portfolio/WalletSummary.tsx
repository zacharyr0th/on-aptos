...
import { PieChart, calculateAssetAllocation, CHART_COLORS } from "./shared/ChartUtils";
...
export const WalletSummary: React.FC<WalletSummaryProps> = ({
  ...
  const portfolioMetrics = usePortfolioMetrics(assets, defiPositions, nfts, nftTotalValue);

  // Calculate totals once and reuse
  const { calculatedTotalValue } = useMemo(() => {
    const tokenTotal = assets.reduce((sum, asset) => sum + (asset.value || 0), 0);
    const defiTotal = (defiPositions || []).reduce((sum, pos) => sum + (pos.totalValue || 0), 0);
    return {
  ...
  const allAssetsData = useMemo(() => 
    calculateAssetAllocation(assets, defiPositions, calculatedTotalValue, CHART_COLORS),
                    ...
                    <div>
                      ...
                      <p className="text-base font-medium mt-1 font-mono">
                        {calculatedTotalValue > 0
                          ? (((selectedAsset.value || 0) / calculatedTotalValue) * 100).toFixed(1)
                          : "0"}%
                      </p>
                    </div>
                  </div>
