...
type PortfolioAction =
  ...
  | { type: 'SET_SIDEBAR_VIEW'; payload: 'assets' | 'nfts' | 'defi' }
  | { type: 'TOGGLE_FILTERED_ASSETS' }
  | { type: 'SET_DEFI_SORT'; payload: { sortBy: 'protocol' | 'value' | 'type'; sortOrder: 'asc' | 'desc' } }
    ...
    case 'SELECT_ITEM':
      return { 
        ...
        sidebarView: action.payload ? (
          action.payload.type === 'asset' ? 'assets' :
          action.payload.type === 'nft' ? 'nfts' : 'defi'
        ) : state.sidebarView
      };
    case 'SET_ACTIVE_TAB':
...
export function PortfolioProvider({ children }: { children: ReactNode }) {
  ...
  const setDeFiSort = useCallback((sortBy: 'protocol' | 'value' | 'type', sortOrder: 'asc' | 'desc') => {
