...
const SUPPORTED_PROTOCOLS: Protocol[] = [
  ...
  // Other DeFi
  {
    ...
    logo: "/icons/protocols/mirage.webp",
    type: "RWA",
...
const ProtocolCategory: React.FC<ProtocolCategoryProps> = ({
  title,
  protocols,
  color,
}) => (
    ...
    <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-2">
      {protocols.map((protocol) => (
        <div key={protocol.id} className="group relative">
          <button
            onClick={() =>
              protocol.website && window.open(protocol.website, "_blank")
            }
...
export const DeFiProtocolShowcase: React.FC = () => {
  ...
  const dexProtocols = SUPPORTED_PROTOCOLS.filter(
    (p) =>
      p.type === "DEX" ||
      p.type === "DEX Aggregator" ||
      p.type === "Order Book DEX",
  );
  const lendingProtocols = SUPPORTED_PROTOCOLS.filter(
    (p) => p.type === "Lending",
  );
  const stakingProtocols = SUPPORTED_PROTOCOLS.filter(
    (p) => p.type === "Liquid Staking",
  );
  const derivativesProtocols = SUPPORTED_PROTOCOLS.filter(
    (p) => p.type === "Derivatives",
  );
  const bridgeProtocols = SUPPORTED_PROTOCOLS.filter(
    (p) => p.type === "Bridge",
  );
