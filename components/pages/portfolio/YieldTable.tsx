...
import { ArrowUpDown, ExternalLink, TrendingUp, ChevronDown, ChevronRight, X, ChevronLeft, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export function YieldTable({ walletAddress }: YieldTableProps) {
  ...
  const loadOpportunities = async () => {
    ...
    try {
      ...
      logger.error(`Failed to load yield opportunities: ${err instanceof Error ? err.message : String(err)}`);
  ...
  const filteredOpportunities = React.useMemo(() => {
    ...
    if (globalFilter) {
      filtered = filtered.filter(o => 
        ...
        o.pairedAssetSymbol?.toLowerCase().includes(globalFilter.toLowerCase())
      );
    }
    
    return filtered;
  }, [opportunities, selectedAsset, selectedProtocol, selectedType, minAPY, globalFilter]);
  ...
  const uniqueProtocols = React.useMemo(() => 
    ['all', ...new Set(opportunities
      .filter(o => 
        !o.protocol.toLowerCase().includes('tortuga') && 
        !o.protocol.toLowerCase().includes('aptin')
      )
      .map(o => o.protocol))].sort(),
    [opportunities]
  );
  ...
  const hasActiveFilters = selectedAsset !== 'all' || 
          ...
          return (
            <div className="text-right">
              <Button
                size="sm"
                variant="ghost"
                onClick={() => safeWindowOpen(`https://${protocol}.fi`, '_blank')}
              >
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="grid grid-cols-7 gap-4 p-4 border-b last:border-b-0">
                  ...
                  <div className="flex items-center gap-1"> {/* Asset pair */}
        ...
        <div className="flex flex-col sm:grid sm:grid-cols-9 gap-2 sm:gap-4 w-full">
          ...
          <div className="grid grid-cols-4 sm:contents gap-2">
            <Button
              ...
              size="sm"
              onClick={() => setSelectedAsset(selectedAsset === 'APT' ? 'all' : 'APT')}
            ...
            <Button
              ...
              size="sm"
              onClick={() => setSelectedAsset(selectedAsset === 'USDT' ? 'all' : 'USDT')}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
              USDT
            </Button>
            <Button
              ...
              size="sm"
              onClick={() => setSelectedAsset(selectedAsset === 'USDC' ? 'all' : 'USDC')}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
              USDC
            </Button>
            <Button
              ...
              size="sm"
              onClick={() => setSelectedAsset(selectedAsset === 'SUSDE' ? 'all' : 'SUSDE')}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
              SUSDE
            </Button>
            <Button
              variant={selectedAsset === 'aBTC' || selectedAsset === 'ABTC' ? 'default' : 'ghost'}
              size="sm"
              onClick={() => {
                if (selectedAsset === 'aBTC' || selectedAsset === 'ABTC') {
                  ...
                  const hasABTC = opportunities.some(o => o.assetSymbol === 'aBTC');
                  const hasABTCCaps = opportunities.some(o => o.assetSymbol === 'ABTC');
                  setSelectedAsset(hasABTC ? 'aBTC' : hasABTCCaps ? 'ABTC' : 'aBTC');
                }
              }}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
            ...
            <Button
              variant={selectedAsset === 'xBTC' || selectedAsset === 'XBTC' ? 'default' : 'ghost'}
              size="sm"
              onClick={() => {
                if (selectedAsset === 'xBTC' || selectedAsset === 'XBTC') {
                  ...
                  // Try both capitalizations to find which one exists in the data
                  const hasXBTC = opportunities.some(o => o.assetSymbol === 'xBTC');
                  const hasXBTCCaps = opportunities.some(o => o.assetSymbol === 'XBTC');
                  setSelectedAsset(hasXBTC ? 'xBTC' : hasXBTCCaps ? 'XBTC' : 'xBTC');
                }
              }}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
            ...
            <Button
              ...
              size="sm"
              onClick={() => setSelectedAsset(selectedAsset === 'WBTC' ? 'all' : 'WBTC')}
              className="text-xs h-10 bg-transparent border border-input hover:bg-accent hover:text-accent-foreground"
            >
              WBTC
            </Button>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-1">
                                  {row.original.rewards.map((reward) => (
                                    <div key={reward.token} className="text-sm">
                                      <span className="font-mono">{reward.symbol}:</span>{" "}
                            ...
                            <div>
                              ...
                              <div className="space-y-1 text-sm">
                                {row.original.apr !== undefined && row.original.apr !== row.original.apy && (
                                  <div>Base APR: {formatPercentage(row.original.apr)}</div>
                                )}
                                {row.original.lockPeriod !== undefined && (
                                  <div>Lock Period: {row.original.lockPeriod === 0 ? "None" : `${row.original.lockPeriod} days`}</div>
                                )}
