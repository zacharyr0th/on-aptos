...
function PortfolioPageContent() {
  ...
  const { validateAddress, normalizeAddress, addressError, setAddressError, clearError } = useAddressValidation();
  const { aptPrice } = useAptPrice();

  const [selectedTimeframe] = useState<"1h" | "12h" | "24h" | "7d" | "30d" | "90d" | "1y" | "all">("1y");
  ...
  logger.debug(`Portfolio page state - connected: ${connected}, walletAddress: ${walletAddress}, normalizedAddress: ${normalizedAddress}, isManualMode: ${state.isManualMode}, manualAddress: ${state.manualAddress}`);
  ...
  const handleItemSelect = useCallback((type: 'asset' | 'nft' | 'defi' | null, data: any) => {
  ...
  return (
    <div className="flex flex-col relative min-h-screen">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-rose-50/40 via-purple-50/30 to-sky-50/40 dark:from-slate-950/95 dark:via-purple-950/20 dark:to-indigo-950/30" />
        
        {/* Soft pastel overlay */}
        <div className="absolute inset-0 bg-pastel-wash" />
        
        {/* Subtle pastel vignette */}
        <div className="absolute inset-0 bg-pastel-vignette" />
        
        {/* Minimal texture */}
        <div className="absolute inset-0 bg-minimal-grain mix-blend-soft-light opacity-[0.015] dark:opacity-[0.02]" />
      </div>

      <main className="w-full px-6 sm:px-8 md:px-12 lg:px-16 xl:px-20 py-4 flex-1 relative z-10">
        <AddressBar
          manualAddress={state.manualAddress}
        ...
        <PortfolioLayout
          ...
          onSidebarViewChange={(view) => dispatch({ type: 'SET_SIDEBAR_VIEW', payload: view as any })}
