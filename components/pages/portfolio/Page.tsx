...
export default function PortfolioPage() {
  ...
  const normalizedAddress = activeAddress && !activeAddress.startsWith("0x") 
    ? `0x${activeAddress}` 
    : activeAddress;
  ...
  useEffect(() => {
    const fetchAptPrice = async () => {
      try {
        ...
        logger.error(`Failed to fetch APT price: ${error instanceof Error ? error.message : String(error)}`);
      }
    };
  ...
  const visibleAssets = useMemo(() => {
    ...
    return assets.filter((asset) => {
      ...
      if (FILTERED_ADDRESSES.length > 0 && FILTERED_ADDRESSES.includes(asset.asset_type)) return false;
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col relative">
        ...
        <main className="container-layout py-2 flex-1 relative">
    ...
    <div className="flex flex-col relative">
      ...
      <main className="container-layout py-6 flex-1 relative">
        ...
        <div className="lg:hidden space-y-8">
          ...
          {/* NFTs Section */}
          {(totalNFTCount !== null && totalNFTCount > 0) || (nfts && nfts.length > 0) ? (
            <div>
