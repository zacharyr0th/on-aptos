...
export const StatsGrid: React.FC<StatsGridProps> = ({
  ...
  if (variant === "bordered") {
    return (
      <div className={`border border-border/30 rounded-lg ${className}`}>
        <div className={`grid ${gridClass[columns]}`}>
          {children}
        </div>
      </div>
    );
  ...
  if (variant === "minimal") {
    return (
      <div className={`grid ${gridClass[columns]} gap-4 sm:gap-6 ${className}`}>
        {children}
      </div>
    );
  }
...
export const ProtocolLogos: React.FC<ProtocolLogosProps> = ({
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      {remainingCount > 0 && (
        <div className={`${sizeClasses[size]} rounded-lg bg-muted border border-border/30 flex items-center justify-center`}>
...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  <div className={`flex flex-col items-center justify-center py-16 ${className}`}>
    ...
    <p className="text-muted-foreground text-center max-w-sm mx-auto mb-4">
      {description}
    </p>
    {action && (
      action.href ? (
