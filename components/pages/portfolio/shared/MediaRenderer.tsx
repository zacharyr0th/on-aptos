...
export const MediaRenderer: React.FC<MediaRendererProps> = ({
    ...
    const detectMediaType = async () => {
      ...
      if (currentSrc.includes('ipfs') || currentSrc.includes('arweave')) {
        try {
          ...
          logger.debug(`Failed to detect media type for ${currentSrc}, defaulting to image`);
          setMediaType("image");
        }
      } else {
        setMediaType("image");
  ...
  const handleError = useCallback((e: React.SyntheticEvent<HTMLElement, Event>) => {
    setHasError(true);
    setIsLoading(false);
  ...
  return (
    <Image
      ...
      priority={priority}
      {...(fill ? { fill: true } : { width: width || 48, height: height || 48 })}
      unoptimized={currentSrc.includes('ipfs') || currentSrc.includes('arweave')}
    />
  );
};

// NFT-specific media renderer
export const NFTMedia: React.FC<Omit<MediaRendererProps, "placeholder">> = (props) => (
