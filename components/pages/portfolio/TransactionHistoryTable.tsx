...
import { ArrowUpDown, ExternalLink, Clock, History, ArrowUpRight, ArrowDownRight, Search, X, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ArrowLeftRight, Coins, Zap, Gift, TrendingUp, Layers, Shield, Wallet } from 'lucide-react';
...
} from '@/lib/utils/enhanced-transaction-analysis';
...
const getTransactionCategory = (tx: Transaction) => {
  ...
  switch (analysis.category) {
    case TransactionCategory.DEFI:
      ...
      if (analysis.activityType === ActivityType.LIQUIDITY_ADD || analysis.activityType === ActivityType.LIQUIDITY_REMOVE) return 'liquidity';
      if (analysis.activityType === ActivityType.LENDING_SUPPLY || analysis.activityType === ActivityType.LENDING_WITHDRAW || 
          analysis.activityType === ActivityType.LENDING_BORROW || analysis.activityType === ActivityType.LENDING_REPAY) return 'liquidity';
      if (analysis.activityType === ActivityType.FARMING_STAKE || analysis.activityType === ActivityType.FARMING_UNSTAKE) return 'staking';
...
export const TransactionHistoryTable: React.FC<TransactionHistoryTableProps> = React.memo(
  ({ walletAddress, className, preloadedTransactions, preloadedTransactionsLoading }) => {
    ...
    const fetchInitialTransactions = async () => {
      ...
      try {
        ...
        // Check cache first
        const cachedData = getCachedData<Transaction[]>('portfolio', cacheKey);
        if (cachedData && cachedData.length > 0) {
          setAllTransactions(cachedData);
          logger.info(`Loaded ${cachedData.length} cached initial transactions for wallet ${walletAddress}`);
        ...
        logger.info(`Loaded initial ${transactions.length} of ${result.totalCount || 'unknown'} transactions for wallet ${walletAddress}`);
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Unknown error';
        setError(errorMessage);
        logger.error(`Transaction fetch error: ${err instanceof Error ? err.message : String(err)}`);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      try {
        ...
        if (cachedData && cachedData.length > 0) {
          ...
          setAllTransactions(prev => {
            const existingVersions = new Set(prev.map(tx => tx.transaction_version));
            const newTransactions = cachedData.filter((tx: Transaction) => !existingVersions.has(tx.transaction_version));
            logger.info(`Loaded ${newTransactions.length} cached remaining transactions (${cachedData.length - newTransactions.length} duplicates filtered)`);
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch remaining transactions: ${response.status}`);
        }
        ...
        // Update pagination state
        setHasMore(result.hasMore || false);
        setCurrentOffset(result.nextOffset || allTransactions.length + newTransactions.length);
        setHasLoadedAll(!result.hasMore);
        ...
        setAllTransactions(prev => {
          const existingVersions = new Set(prev.map(tx => tx.transaction_version));
          const uniqueTransactions = newTransactions.filter((tx: Transaction) => !existingVersions.has(tx.transaction_version));
          logger.info(`Loaded ${uniqueTransactions.length} more transactions (total: ${prev.length + uniqueTransactions.length}/${totalCount})`);
          return [...prev, ...uniqueTransactions];
        });
        
        logger.info(`Loaded ${newTransactions.length} additional transactions for wallet ${walletAddress}`);
      } catch (err) {
        logger.error(`Error loading remaining transactions: ${err instanceof Error ? err.message : String(err)}`);
    ...
    const filteredTransactions = React.useMemo(() => {
      ...
      if (searchQuery) {
        ...
        filtered = filtered.filter((tx) => {
          ...
          const activityType = EnhancedTransactionAnalyzer.getActivityTypeDisplayName(analysis.activityType).toLowerCase();
    ...
    React.useEffect(() => {
      if (preloadedTransactions !== null && preloadedTransactions !== undefined) {
        // Use preloaded transactions
        setAllTransactions(preloadedTransactions || []);
        setIsLoading(preloadedTransactionsLoading || false);
        logger.info(`Using ${(preloadedTransactions || []).length} preloaded transactions for wallet ${walletAddress}`);
      ...
      () => [
        ...
        },
        {
          ...
          cell: ({ row }) => {
            ...
            const amount = transaction.asset_type === 'APT' 
            ...
            return (
              <div className="flex items-center gap-2 w-[280px] flex-shrink-0">
                ...
                >
                  {(() => {
                    const logoPath = analysis.protocol?.name ? getProtocolLogoPath(analysis.protocol.name) : null;
                    return logoPath ? (
                      <img 
                        src={logoPath} 
                  ...
                  <IconComponent
                    className={cn(
                      'h-4 w-4',
                      analysis.protocol?.name && getProtocolLogoPath(analysis.protocol.name) ? 'hidden' : 'block',
                      isIncoming ? 'text-green-600 dark:text-green-400' : isOutgoing ? 'text-red-600 dark:text-red-400' : 'text-muted-foreground'
                    )}
                  />
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  {!analysis.protocol && transaction.type && !transaction.type.includes('transfer_coins') && (
          ...
          cell: ({ row }) => {
            ...
            const getCategoryBadgeInfo = () => {
              ...
              switch (analysis.category) {
                case TransactionCategory.DEFI:
                  return { 
                    label: EnhancedTransactionAnalyzer.getActivityTypeDisplayName(analysis.activityType), 
                    colors 
                  };
                case TransactionCategory.STAKING:
                  return { 
                    label: analysis.activityType === ActivityType.CLAIM_REWARDS ? 'Rewards' : 'Staking', 
                    colors 
                  };
                case TransactionCategory.CEX:
      ...
      return (
        <div className={className}>
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-base font-medium">{t('transactions.history') || 'Transaction History'}</h2>
          </div>
              ...
              return (
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <Badge variant="outline" className={cn("text-xs border flex-shrink-0", badgeInfo.colors)}>
                          {badgeInfo.label}
                        </Badge>
                      ...
                      <div className="text-right flex-shrink-0 ml-2">
                        <div className="text-xs whitespace-nowrap">{date.toLocaleDateString()}</div>
                    ...
                    {/* Asset badges - Fixed height container */}
                    <div className="min-h-[24px] flex items-start">
                      {analysis.assetInfo && (analysis.assetInfo.isStablecoin || analysis.assetInfo.isLST || analysis.assetInfo.isRWA) && (
          ...
          {filteredTransactions.length > 10 && (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <div className="flex-1 text-sm text-muted-foreground text-left">
                ...
                {totalCount > 0 && (
                  <span className="ml-2">• Showing {allTransactions.length} of {totalCount.toLocaleString()} total transactions</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                <Button
