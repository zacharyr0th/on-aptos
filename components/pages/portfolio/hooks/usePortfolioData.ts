...
export function usePortfolioData(
  ...
  const loadRemainingTransactions = async (startOffset: number) => {
    ...
    try {
      ...
      logger.debug(`Starting background loading of remaining transactions from offset ${startOffset} for wallet:`, walletAddress);
      ...
      while (hasMore) {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch transactions at offset ${offset}: ${response.status}`);
        }
        ...
        if (batchTransactions.length === 0) {
          ...
          logger.debug(`Loaded batch: ${batchTransactions.length} transactions (total: ${allTransactions.length}) for wallet ${walletAddress}`);
      ...
      logger.info(`Completed loading all ${allTransactions.length} transactions for wallet ${walletAddress}`);
      
    } catch (error) {
      logger.error(`Failed to load remaining transactions: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      logger.info(`Loaded first ${firstBatch.length} transactions immediately for wallet ${walletAddress}`);
      ...
      logger.info(`Completed loading ${allTransactions.length} transactions in background for wallet ${walletAddress}`);
      
    } catch (error) {
      logger.error(`Failed to load background transactions: ${error instanceof Error ? error.message : String(error)}`);
      // Don't set error for background loading - just log it
     ...
     walletAddress });
      ...
      try {
        ...
        if (useBatchApi) {
          ...
          logger.info(`BATCH API RESPONSE - transactions: ${data.transactions ? data.transactions.length : 'null'}, hasMoreTransactions: ${data.hasMoreTransactions}`);
          
          if (data.transactions && data.transactions.length > 0) {
            setTransactions(data.transactions);
            setTransactionsLoading(false); // Mark as loaded
            logger.info(`✅ INSTANT LOAD: ${data.transactions.length} transactions loaded immediately from batch API for wallet ${walletAddress}`);
            
            // If there might be more transactions, load them in background
            if (data.hasMoreTransactions) {
              logger.info(`📋 BACKGROUND LOAD: Starting to load remaining transactions after ${data.transactions.length}`);
              loadRemainingTransactions(data.transactions.length);
            } else {
              logger.info(`✅ ALL LOADED: All ${data.transactions.length} transactions loaded from batch API`);
            }
          } else {
          ...
          if (assetsResponse.ok) {
            try {
              ...
              logger.error(`Assets parsing error: ${error instanceof Error ? error.message : String(error)}`);
            }
          }
          ...
          try {
            ...
            logger.error(`DeFi fetch failed: ${error instanceof Error ? error.message : String(error)}`);
          } finally {
          ...
          try {
            ...
            logger.error(`Failed to fetch NFTs: ${nftError instanceof Error ? nftError.message : String(nftError)}`);
          } finally {
        ...
        logger.error(`Failed to fetch portfolio data: ${err instanceof Error ? err.message : String(err)}`);
  ...
  const loadMoreNFTs = async () => {
    ...
    try {
      ...
      logger.error(`Failed to load more NFTs: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
    ...
    try {
      if (useBatchApi) {
        ...
        if (data.transactions) {
          setTransactions(data.transactions);
          logger.info(`Reloaded ${data.transactions.length} transactions from batch API for wallet ${walletAddress}`);
        ...
        try {
          ...
          logger.error(`Failed to refetch NFTs: ${nftError instanceof Error ? nftError.message : String(nftError)}`);
          setNfts([]);
          setAllNFTs([]);
        }
