    ...
    const fetchPortfolioPerformance = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          logger.error(`Portfolio performance API error: ${errorData instanceof Error ? errorData.message : String(errorData)}`);
          throw new Error(
        ...
        logger.error(`Portfolio performance error: ${err instanceof Error ? err.message : String(err)}`);
