...
export function useAnalytics(
  ...
  const fetchAnalytics = useCallback(async () => {
    if (!walletAddress) {
      logger.debug("[useAnalytics] No wallet address provided");
      setData({
        portfolioPerformance: null,
        balanceHistory: null,
        tokenLatestPrice: null,
    ...
    logger.debug("[useAnalytics] Fetching analytics for wallet:", {
      walletAddress,
    });
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      logger.debug("[useAnalytics] Analytics data fetched:", {
