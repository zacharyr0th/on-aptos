...
import { PortfolioAsset, NFT, DeFiPosition, Transaction, PortfolioMetrics, ChartDataPoint } from "@/lib/portfolio/types";
...
const Sidebar = memo(({ data, ui, actions }: Pick<CleanLayoutProps, 'data' | 'ui' | 'actions'>) => (
  <aside className="w-80 space-y-4">
    <div className="bg-card rounded-lg p-4">
      <h2 className="text-2xl font-bold">${data.totalValue.toLocaleString()}</h2>
...
const MainContent = memo(({ data, ui, actions }: Pick<CleanLayoutProps, 'data' | 'ui' | 'actions'>) => (
        ...
        {(['portfolio', 'transactions', 'yield'] as const).map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-lg capitalize transition-colors ${
              ui.activeTab === tab ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'
            }`}
          >
            {tab}
          </button>
          ...
          {data.assets.map(asset => (
            <div
              ...
              className={`p-4 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors ${
                ui.selectedItem?.type === 'asset' && ui.selectedItem.data === asset ? 'border-primary' : ''
              }`}
            >
              <div className="flex justify-between">
                <div>
                  <p className="font-medium">{asset.metadata?.symbol}</p>
                  <p className="text-sm text-muted-foreground">{asset.metadata?.name}</p>
                </div>
                <div className="text-right">
                  <p className="font-medium">${asset.value?.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">{asset.balance} {asset.metadata?.symbol}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {ui.activeTab === 'transactions' && (
        <div className="space-y-2">
          {data.transactions.slice(0, 20).map(tx => (
            <div key={tx.transaction_version} className="p-3 rounded-lg border">
              <div className="flex justify-between">
