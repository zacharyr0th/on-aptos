...
import type { NFT } from "@/lib/types/consolidated";

// Local variant with specific fields for this component
interface LocalNFT extends NFT {
  ...
  // Ensure all required NFT properties are available
  amount: number;
}

interface NFTAnalysisProps {
  nfts: LocalNFT[];
  nftsByCollection: Record<string, LocalNFT[]>;
  selectedNFT: LocalNFT | null;
  onClearSelection?: () => void;
}

export function NFTAnalysis({
  nfts,
  nftsByCollection: _nftsByCollection,
  selectedNFT,
  onClearSelection,
}: NFTAnalysisProps) {
  ...
  const _explorerUrl = `https://explorer.aptoslabs.com/account/${displayNFT.token_data_id}?network=mainnet`;
