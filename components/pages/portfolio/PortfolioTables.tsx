        ...
        <>
          ...
          {isMobile ? (
            <div className="space-y-3">
              {aptAsset && <AssetCard asset={aptAsset} showDivider={otherAssets.length > 0} />}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <span>•</span>
                <span>{groupedPosition.positions?.length || 1} position{groupedPosition.positions?.length !== 1 ? 's' : ''}</span>
              </div>
              {protocolInfo?.href && (
                <a
      ...
      {isMobile ? (
        <div className="space-y-3">
          {sortedPositions.map((groupedPosition, index) => (
            <DeFiCard key={`${groupedPosition.protocol}-${index}`} groupedPosition={groupedPosition} index={index} />
          ))}
        </div>
      ) : (
        /* Desktop Table Layout */
