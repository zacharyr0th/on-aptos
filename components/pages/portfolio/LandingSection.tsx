...
import { AnsService } from "@/lib/services/ans";
import { useTranslation } from "@/hooks/useTranslation";

// Available logos from specified directories
const btcLogos = [
  { src: "/icons/btc/WBTC.png", alt: "WBTC" },
  { src: "/icons/btc/bitcoin.png", alt: "Bitcoin" },
  { src: "/icons/btc/echo.png", alt: "Echo BTC" },
  { src: "/icons/btc/okx.png", alt: "OKX BTC" },
  { src: "/icons/btc/stakestone.png", alt: "StakeStone BTC" },
];

const stableLogos = [
  { src: "/icons/stables/USDA.png", alt: "USDA" },
  { src: "/icons/stables/mUSD.svg", alt: "mUSD" },
  { src: "/icons/stables/mod.png", alt: "MOD" },
  { src: "/icons/stables/susde.png", alt: "sUSDe" },
  { src: "/icons/stables/usdc.png", alt: "USDC" },
  { src: "/icons/stables/usde.png", alt: "USDe" },
  { src: "/icons/stables/usdt.png", alt: "USDT" },
];
    ...
    if (AnsService.isAptDomain(inputAddress)) {
      ...
      try {
        ...
        if (result.address) {
          ...
          setAddressError(`Could not resolve ${inputAddress}. The domain may not exist or has expired.`);
        }
      } catch (error) {
              ...
              <div className="space-y-6 text-left">
                <div>
                  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.05] tracking-tight">
                    ...
                    <span className="block text-foreground">{t("portfolio.overview", "Overview")}</span>
                  </h1>
                </div>

                <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed max-w-full md:max-w-lg">
                  {t("portfolio.landing.subtitle_desc", "Track all your assets, DeFi positions, and NFTs in real-time across the Aptos ecosystem")}
                </p>

                {/* CTA Section with Wallet Connect and Address Input */}
                <div className="space-y-4 pt-2">
                  <div className="space-y-4 max-w-xs">
                    <WalletConnectButton size="lg" className="h-12 px-6 text-base w-full" />
                    ...
                    <div className="relative">
                      ...
                      />
                      {addressError && (
                        <p className="text-sm text-destructive mt-2">{addressError}</p>
                      )}
                    </div>
                  </div>
                </div>
