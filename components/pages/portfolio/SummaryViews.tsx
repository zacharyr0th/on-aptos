...
export const NFTSummaryView: React.FC<NFTSummaryViewProps> = ({
  ...
  const collections = React.useMemo((): [string, number][] => {
    if (nftCollectionStats && nftCollectionStats.collections.length > 0) {
      ...
      return nftCollectionStats.collections.map(({ name, count }) => [
        name,
        count,
      ] as [string, number]);
    } else {
          ...
          <div className="p-4 space-y-3">
            ...
            {selectedNFT.description && (
              <div className="text-sm text-muted-foreground">
                ...
                <p className="line-clamp-3">{selectedNFT.description}</p>
              </div>
            )}
              ...
              onClick={(data: any) => {
                if (onCollectionClick && data && data.name) {
                  ...
                  const originalName = collections.find(
                    ([name]: [string, number]) => (name || "Unnamed Collection") === data.name,
                  )?.[0];
      ...
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-6">
            ...
            <div className="bg-card border rounded-lg p-3 sm:p-4">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <p className="text-lg sm:text-xl font-semibold">
                  {
                    collections.filter(([_, count]: [string, number]) => count === 1)
                      .length
                  }
                </p>
              </div>
            ...
            <div className="bg-card border rounded-lg p-3 sm:p-4">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <p className="text-lg sm:text-xl font-semibold">
                  {Math.round(
                    ((collections[0]?.[1] || 0) / actualNFTCount) *
                      100,
                  )}
                  %
                </p>
            ...
            <div className="bg-card border rounded-lg p-3 sm:p-4">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <p className="text-lg sm:text-xl font-semibold">
                  {
                    collections.filter(([_, count]: [string, number]) => count >= 5)
                      .length
                  }
                </p>
              </div>
            ...
            <div className="bg-card border rounded-lg p-3 sm:p-4">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <p className="text-lg sm:text-xl font-semibold">
                  {
                    collections.filter(([_, count]: [string, number]) => count >= 10)
                      .length
                  }
                </p>
              </div>
        ...
        <div className="lg:hidden">
          <button
            ...
            className="w-full flex items-center justify-between p-4 bg-card border rounded-lg hover:bg-muted/50 transition-colors"
          >
                  ...
                  <p className="text-lg sm:text-xl font-semibold">
                    {(
                      (collections
                        .slice(0, 3)
                        .reduce(
                          (sum: number, [_, count]: [string, number]) => sum + count,
                          0,
                        ) /
                        actualNFTCount) *
                      100
