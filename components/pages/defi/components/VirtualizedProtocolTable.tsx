  ...
  return (
    <div style={style} className="border-b">
      <div className="flex items-center h-full px-4 md:px-8 hover:bg-muted/50 transition-colors">
        {/* Protocol Info */}
        <div className={`flex gap-4 min-w-0 ${isMobile ? 'flex-col items-center text-center' : 'items-center'} ${isMobile ? 'w-full' : 'w-1/4'}`}>
          {/* Protocol Logo */}
          <div className="flex-shrink-0">
            {protocol.logo ? (
              <div className={`relative rounded-full overflow-hidden bg-secondary ring-2 ring-border/20 ${isMobile ? 'w-16 h-16' : 'w-10 h-10'}`}>
                <Image
                  src={protocol.logo}
              ...
              <div className={`rounded-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 flex items-center justify-center ring-2 ring-border/20 ${isMobile ? 'w-16 h-16' : 'w-10 h-10'}`}>
                <span className={`font-bold text-gray-600 dark:text-gray-300 ${isMobile ? 'text-xl' : 'text-sm'}`}>
                  {protocol.title.charAt(0)}
                </span>
              </div>
            )}
          </div>
          <div className={`min-w-0 flex-1 ${isMobile ? 'space-y-3 w-full' : ''}`}>
            <div className={`font-bold truncate ${isMobile ? 'text-xl mb-2' : 'text-base'}`}>
              {protocol.title}
            </div>
        ...
        {!isMobile && (
          <div className="w-1/4 px-6">
            <div className="flex flex-wrap gap-2">
              {protocol.subcategory.split(', ').map((subcategory: string, subIndex: number) => (
                <span
                  key={subIndex}
...
export const VirtualizedProtocolTable = React.memo(function VirtualizedProtocolTable({
  ...
  const getSocialLinks = useCallback((protocol: (typeof defiProtocols)[0]) => {
    const links = [];
      ...
      <div className="rounded-xl border bg-card overflow-hidden shadow-sm">
        ...
        <div className="border-b-2 bg-card">
          <div className="flex items-center hover:bg-transparent">
            <div className={`font-bold text-base py-4 md:py-6 px-4 md:px-8 ${isMobile ? 'w-full text-center' : 'w-1/4'}`}>
