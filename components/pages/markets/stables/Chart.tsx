...
import type { DisplayToken } from "@/lib/config/tokens";
...
import { errorLogger, logger } from "@/lib/utils/logger";
...
export interface MarketShareChartProps {
  data: DisplayToken[];
  totalSupply: string;
  tokenMetadata?: Record<string, { assetAddress: string; name?: string; decimals?: number }>;
  susdePrice?: number;
  ...
  try {
    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground">Market Share: {formatPercentage(value)}%</p>
...
export const MarketShareChart = memo<MarketShareChartProps>(
  ({ data, totalSupply, tokenMetadata = {}, susdePrice, showTotalInfo = true }) => {
    const { isMobile, isDesktop } = useResponsive();
    const width = isDesktop ? 1024 : isMobile ? 320 : 768;

    logger.debug(
      `[MarketShareChart] Props: totalSupply=${totalSupply}, susdePrice=${susdePrice}, showTotalInfo=${showTotalInfo}`
    );
      ...
      return measurePerformance(() => {
        try {
          const values = data.map((token) => {
            ...
            // Check if token has 8 decimals (MOD, USDA)
            const decimals = token.symbol === "MOD" || token.symbol === "USDA" ? 8 : 6;
          ...
          errorLogger.error(`Error calculating token stable values: ${error}`);
          return [];
        }
    ...
    const chartData = useMemo<ChartDataItem[]>(() => {
      ...
      return measurePerformance(() => {
        try {
          ...
          const totalUSDValue = tokenStableValues.reduce((sum, item) => sum + item.usdValue, 0);
          ...
          for (const item of tokenStableValues) {
            ...
            const marketShare = totalUSDValue > 0 ? (item.usdValue / totalUSDValue) * 100 : 0;
          ...
          // Group items under 2% into "Other" category for chart display
          const chartItems: ChartDataItem[] = [];
          const otherItems: ChartDataItem[] = [];

          for (const token of sortedTokens) {
            // Keep tokens with 2% or more market share
            if (token.value >= 2.0) {
          ...
          result.push(...chartItems);

          // If we have items for "Other" category, combine them
          if (otherItems.length > 0) {
            const otherValue = otherItems.reduce((sum, token) => sum + token.value, 0);
          ...
          // Debug logging
          logger.info(
            `Stablecoin chart data processed: totalTokens=${allTokens.length}, chartItems=${chartItems.length}, otherItems=${otherItems.length}`
          );
    ...
    const getCellColor = useCallback((entry: ChartDataItem) => {
      if (entry.name === "Other") {
        return "#71717a"; // Darker gray color for "Other" category
      }
