...
const getProviderAverageColor = (provider: string, assets: ProtocolData[]): string => {
  // Get base colors for this provider's assets
  const colors = assets.map((asset) => RWA_COLORS[asset.assetTicker] || RWA_COLORS.default);
  ...
  try {
    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground">Share: {formatPercentage(data.value)}%</p>
        {!isProvider && (
...
const CustomLegend = memo<{
  ...
  const remainingPercentage = remainingProviders.reduce((sum, provider) => sum + provider.value, 0);

  return (
    <div className="flex flex-col gap-4">
      {itemsToShow.map((provider, i) => {
        const displayValue = Number.isFinite(provider.value) ? provider.value.toFixed(2) : "0.00";

        return (
          <div key={`legend-${provider.name}-${i}`} className="flex items-center gap-3">
            <div
...
export const MarketShareChart = memo<MarketShareChartProps>(({ data, totalValue }) => {
  ...
  const { providerData, assetData } = useMemo(() => {
    ...
    data.forEach((protocol) => {
      ...
      const provider = getActualProvider(protocol.assetTicker, protocol.protocol);
    ...
    const allProviders: ProviderDataItem[] = Array.from(providerTotals.entries())
      .map(([provider, value]) => {
    ...
    if (otherProviders.length > 0) {
      const otherValue = otherProviders.reduce((sum, provider) => sum + provider.value, 0);
    ...
    chartProviders.forEach((providerItem) => {
      ...
      const providerAssetList = providerAssets.get(provider) || [];
      const sortedAssets = providerAssetList.sort((a, b) => b.totalValue - a.totalValue);
      ...
      sortedAssets.forEach((asset, index) => {
        ...
        if (provider === "Pact" && index === 0) {
          ...
          darkenedColor = getProviderAverageColor(provider, providerAssetList);
        } else {
          // For other assets, apply normal darkening logic
          const darkenFactor =
            sortedAssets.length > 1 ? 0.4 - (index * 0.4) / (sortedAssets.length - 1) : 0;
  ...
  return (
    <div className="relative w-full h-full">
      <div className={`flex items-center justify-center w-full h-full ${isMobile ? "p-4" : "p-6"}`}>
        <div
