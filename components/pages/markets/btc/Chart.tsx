  ...
  try {
    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground">Market Share: {formatBTCPercentage(value)}%</p>
...
export const MarketShareChart = memo<MarketShareChartProps>(
  ({ data, tokenMetadata = {}, bitcoinPrice, totalBTC, totalUSD, showTotalInfo = true }) => {
    ...
    const chartData = useMemo<ChartDataItem[]>(() => {
      ...
      return measurePerformance(() => {
        try {
          ...
          const totalBTCValue = tokenBtcValues.reduce((sum, item) => sum + item.btcValue, 0);
          ...
          if (otherItems.length > 0) {
            ...
            const otherBtcValue = otherItems.reduce((sum, token) => sum + token._btcValue, 0);
            ...
            <div className={`${chartConfig.size} flex-shrink-0 relative`}>
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center">
                  ...
                  >
                    {totalUSD ? formatCurrency(totalUSD, "USD", { decimals: 0 }) : "$0"}
                  </div>
                </div>
              </div>
            </div>
