...
export const ResponsiveTabs: React.FC<ResponsiveTabsProps> = ({
  ...
  const borderClass = showBorder ? "border-b border-neutral-200 dark:border-neutral-800" : "";
  const baseTabClass =
    variant === "borderless"
  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className={`w-full ${className}`}>
      ...
      <div className={`flex justify-between items-end w-full ${borderClass}`}>
        ...
        {eyeToggle && (
          <div className="flex items-center pb-3">
            ...
            >
              {eyeToggle.show ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </Button>
          </div>
        )}
      </div>
      ...
      {/* Tab contents */}
      {tabs.map((tab) => (
        <TabsContent key={tab.value} value={tab.value} className={`mt-6 p-0 ${contentClassName}`}>
          {tab.content}
        </TabsContent>
      ))}
    </Tabs>
...
export const MainContentTabs: React.FC<MainContentTabsProps> = ({
  ...
  return (
    <div className={hideOnMobile ? "hidden lg:block" : ""}>
      <Tabs value={activeTab} onValueChange={onTabChange} className={`h-full ${className}`}>
