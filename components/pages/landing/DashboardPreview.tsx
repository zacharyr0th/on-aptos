    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          logger.error(`Failed to fetch DeFi TVL: ${defiError instanceof Error ? defiError.message : String(defiError)}`);
          // Fallback to internal API if available
        }
      } catch (error) {
        logger.error(`Failed to fetch dashboard data: ${error instanceof Error ? error.message : String(error)}`);
      } finally {
        setLoading(false);
      }
    };
