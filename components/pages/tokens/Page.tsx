...
export default function TokensPage() {
  ...
  const getTokenCategory = (token: TokenData) => {
    ...
    if (
      token.panoraTags &&
      Array.isArray(token.panoraTags) &&
      token.panoraTags.length > 0
    ) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Fetch stablecoin data with correct supplies first
      let stablecoinData: any = {};
      try {
        const stablesResponse = await fetch("/api/aptos/stables");
        if (stablesResponse.ok) {
          const stablesResult = await stablesResponse.json();
          if (stablesResult.success && stablesResult.data?.supplies) {
            // Convert to a lookup map by address
            stablecoinData = stablesResult.data.supplies.reduce(
              (acc: any, stable: any) => {
                acc[stable.asset] = {
                  supply: parseFloat(stable.supply),
                  supply_raw: stable.supply_raw,
                };
                return acc;
              },
              {},
      ...
      // Fetch token list from Panora API
      // Get all tokens to allow flexible filtering
      const panoraResponse = await fetch(
        "https://api.panora.exchange/tokenlist?panoraUI=true,false",
        {
          headers: {
            "x-api-key":
      ...
      if (!panoraResponse.ok) {
        throw new Error("Failed to fetch Panora token list");
      }

      const panoraData = await panoraResponse.json();

      // Panora returns {status: 200, data: [...]}
      const panoraTokens: any[] =
        panoraData.data && Array.isArray(panoraData.data)
          ? panoraData.data
          : [];

      if (panoraTokens.length === 0) {
        throw new Error("No tokens received from Panora API");
      }

      // Fetch real supply data from Aptos indexer for all tokens
      let supplyData: any = {};
      try {
        // Prepare token addresses for supply fetching
        const tokenAddresses = panoraTokens.map((token: any) => ({
          faAddress: token.faAddress,
          tokenAddress: token.tokenAddress,
        }));

        // Fetch supplies from our new endpoint with timeout
        const supplyResponse = await Promise.race([
          fetch("/api/tokens/supplies", {
            method: "POST",
            headers: {
          ...
          new Promise<Response>((_, reject) =>
            setTimeout(() => reject(new Error("Supply fetch timeout")), 30000),
          ),
        ]);

        if (supplyResponse.ok) {
          const supplyResult = await supplyResponse.json();
          if (supplyResult.success && supplyResult.supplies) {
            supplyData = supplyResult.supplies;
          }
        }
      } catch (err) {
        // Continue with empty supply data - tokens will show without FDV
      }

      // Process and enrich tokens
      const processedTokens = panoraTokens.map((token: any) => {
        const price = parseFloat(token.usdPrice || "0");

        // Get supply from stablecoin data first, then fall back to indexer data
        let supply = 0;
        const tokenKey = token.faAddress || token.tokenAddress;

        if (stablecoinData[tokenKey]) {
          // Use correct stablecoin supply (already adjusted for reserves, etc.)
          supply = stablecoinData[tokenKey].supply;
        } else if (token.faAddress && supplyData[token.faAddress]) {
          supply = supplyData[token.faAddress];
        } else if (token.tokenAddress && supplyData[token.tokenAddress]) {
          supply = supplyData[token.tokenAddress];
        }

        // Calculate FDV using real supply data
        const fdv = price * supply;

        // Ensure panoraTags is preserved properly
        const panoraTags = Array.isArray(token.panoraTags)
          ? token.panoraTags
          : [];
        const isVerified = panoraTags.includes("Verified");
      ...
      // Sort by FDV descending by default
      const sortedTokens = processedTokens
        .filter((token) => !token.isBanned)
      ...
      // Create summary data for compatibility
      const summary = {
        total_tokens: sortedTokens.length,
        tokens_with_supply: sortedTokens.filter((t) => (t.supply || 0) > 0)
          .length,
        tokens_analyzed: sortedTokens.length,
        tokens_returned: sortedTokens.length,
        total_market_cap_with_apt: sortedTokens.reduce(
          (sum, t) => sum + (t.fdv || 0),
          0,
        ),
        total_market_cap_non_apt: sortedTokens
          .filter((t) => t.symbol !== "APT")
          .reduce((sum, t) => sum + (t.fdv || 0), 0),
        apt_market_cap: sortedTokens.find((t) => t.symbol === "APT")?.fdv || 0,
      ...
      setData({
        summary,
        distribution: {
          above1B: sortedTokens.filter((t) => (t.fdv || 0) > 1000000000).length,
          from100MTo1B: sortedTokens.filter(
            (t) => (t.fdv || 0) > 100000000 && (t.fdv || 0) <= 1000000000,
          ).length,
          from10MTo100M: sortedTokens.filter(
            (t) => (t.fdv || 0) > 10000000 && (t.fdv || 0) <= 100000000,
          ).length,
          from1MTo10M: sortedTokens.filter(
            (t) => (t.fdv || 0) > 1000000 && (t.fdv || 0) <= 10000000,
          ).length,
          from100KTo1M: sortedTokens.filter(
            (t) => (t.fdv || 0) > 100000 && (t.fdv || 0) <= 1000000,
          ).length,
          below100K: sortedTokens.filter((t) => (t.fdv || 0) <= 100000).length,
        },
        categories: {
  ...
  const displayMetrics = useMemo(() => {
    ...
    if (!showUnverified) {
      ...
      relevantTokens = relevantTokens.filter((token) =>
        token.panoraTags?.includes("Verified"),
      );
    }
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="text-muted-foreground text-sm">{actualIndex}</span>
          );
        },
        size: 60,
      },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="font-medium text-sm">{formatNumber(supply)}</span>
          );
        },
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="font-medium text-sm">{formatCurrency(fdv)}</span>
          );
        },
      },
    ],
      ...
      <main className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20 py-8 flex-1 relative">
        ...
        {(selectedTags.length > 0 ||
          ...
          <div className="flex flex-wrap items-center gap-2 pb-4">
            ...
            {globalFilter && (
              <Badge variant="secondary">Search: {globalFilter}</Badge>
            )}
            ...
            {!includeStables && (
              <Badge variant="secondary">Exclude Stables</Badge>
            )}
            {showUnverified && (
              <Badge variant="secondary">Including Unverified</Badge>
            )}
            <Button
              variant="ghost"
              size="sm"
