...
const PAGE_SIZE = 5000; // Load all tokens to show verified ones first

async function fetchTokens({ pageParam = 0 }): Promise<TokensResponse> {
  const response = await fetch(
    `/api/aptos/tokens?limit=${PAGE_SIZE}&offset=${pageParam}`,
  );
...
function TokenRow({
  ...
  const TrendIcon =
    (token.priceChange24H || 0) >= 0 ? TrendingUp : TrendingDown;
            ...
            <div className="text-right">
              ...
              {token.priceChange24H !== undefined &&
                token.priceChange24H !== 0 && (
                  <div
                    className={`flex items-center justify-end gap-1 text-xs ${priceChangeColor}`}
                  >
...
export function VirtualizedTokenList() {
  ...
  useEffect(() => {
    const updateHeight = () => {
      setListHeight(Math.max(600, window.innerHeight - 400));
    };
    updateHeight();
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-2xl">
            All Tokens ({formatNumber(totalTokens)})
          </CardTitle>
