...
import { Wallet, LogOut, Copy, Check, Apple, Mail } from "lucide-react";
...
export function WalletConnectButton({
  ...
  const handleAptosConnect = async () => {
    ...
    try {
      ...
      logger.warn("Failed to connect with Aptos Connect:", error);
    } finally {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuLabel>{t("wallet.my_account", "My Account")}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        {primaryName && (
          <DropdownMenuItem className="flex flex-col items-start gap-1 cursor-default">
            ...
            <span className="text-xs text-muted-foreground">{t("wallet.ans_name", "ANS Name")}</span>
          </DropdownMenuItem>
        )}
        <DropdownMenuItem
          onClick={copyAddress}
        ...
        >
          <div className="flex flex-col items-start">
            ...
            <span className="text-xs text-muted-foreground">{t("actions.copy_address", "Copy address")}</span>
          </div>
          {copiedAddress ? (
